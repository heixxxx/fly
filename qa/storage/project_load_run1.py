...
# 记录 db_id 供 run2 比对（load 后 db_id 应不变）。
with open(os.path.join(PROJ_PATH, "_run1_db_id"), "w") as f:
    f.write(db.get_db_id())

get_agent().stop()
print(f"[RUN1] make_db done: db_id={db.get_db_id()}, val=12345", file=sys.stderr)
