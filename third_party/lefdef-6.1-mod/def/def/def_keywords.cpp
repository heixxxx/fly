...
defrData::reload_buffer() {
   ...
   if (first_buffer) {
      ...
      if (settings->ReadFunction) {
         if ((nb = (int) (*settings->ReadFunction)(File, buffer, 4)) != 4) {
            next = NULL;
            return;
         }
      } else {
         if ((nb = (int) fread(buffer, 1, 4, File)) != 4) {
            next = NULL;
            return;
         }
      }
   ...
   if (nb == 0) {
      if (settings->ReadFunction)
         nb = (int) (*settings->ReadFunction)(File, buffer, IN_BUF_SIZE);
      else
         ...
         if (K_HISTORY == result) {  /* history - get up to ';' */
            ...
            if (foundTag) {
               ...
               while (1) {
                  ...
                  size_t histTextSize = History_text.size();
...
defrData::defError(int msgNum, const char *s) {
   ...
   const char  *pvToken = isgraph(pv_deftoken[0]) ? pv_deftoken
                                                           : "<unprintable>";
   int len = (int) strlen(curToken)-1;
   int pvLen = (int) strlen(pvToken)-1;
   ...
   int msgIdx = msgNum - 5000;

   if (msgIdx >= 0 && msgIdx < DEF_MSGS && settings->MsgLimit[msgIdx] > 0) {
       if (msgLimit[msgIdx] >= settings->MsgLimit[msgIdx]) {
           return;             /* over the limit */
       }

        msgLimit[msgIdx] = msgLimit[msgIdx] + 1;
   }
   ...
   if (strcmp(s, "parse error") == 0) {
      if ((len > 1) && (deftoken[len] == ';')) {
         ...
         str = (char*)malloc((size_t) len + strlen(session->FileName) + 350);
