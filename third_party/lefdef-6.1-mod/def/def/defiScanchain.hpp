...
public:
  ...
  const defiProp* prop(int index) const;// 6.0
...
public:
  ...
  void startOrderedList();                  // 6.0
  void closeOrderedList();                  // 6.0
  int  hasOpenedOrderedList();              // 6.0
  void addOrderedInst(const char* inst);
  void addOrderedIn(const char* inPin);
  void addOrderedOut(const char* outPin);
  void setOrderedBits(int bits);            // 5.4.1
  void addOrderedProp(defiProp* prop);      // 6.0
  void setOrderedName(const char* name);    // 6.0
  ...
  int numOrderedProps(int index) const;                         // 6.0
  const defiProp*   orderedProp(int index, int propIdx) const;  // 6.0
  const char*       orderedName(int index);                     // 6.0
