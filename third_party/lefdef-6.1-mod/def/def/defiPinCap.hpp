...
public:
  ...
  void addVia(const char* via, int viaX, int viaY, int color = 0, int orient = 0);
  void setPlacement(int typ, int x, int y, int orient);
  void setLayerPlacement(int typ, int x, int y, int orient);
  void setPolygonPlacement(int typ, int x, int y, int orient);
  void addLayerProp(defiProp* prop);                          // 6.0
  void addPolyProp(defiProp* prop);                           // 6.0
  ...
  int hasLayerPlacement(int index) const;
  int layerPlacementType(int index) const;
  int layerPlacementX(int index) const;
  int layerPlacementY(int index) const;
  int layerPlacementOrient(int index) const;
  const char* layerPlacementOrientStr(int index) const;
  int hasPolygonPlacement(int index) const;
  int polygonPlacementType(int index) const;
  int polygonPlacementX(int index) const;
  int polygonPlacementY(int index) const;
  int polygonPlacementOrient(int index) const;
  const char* polygonPlacementOrientStr(int index) const;
  int   numVias() const;
  ...
  int   numLayerProps(int layerIdx) const;
  int   numPolyProps(int polydx) const;
  int   numViaProps(int viaIdx) const;
  const defiProp* layerProp(int layerIdx, int propIdx) const;
  const defiProp* polyProp(int polyIdx, int propIdx) const;
  const defiProp* viaProp(int viaIdx, int propIdx) const;
...
protected:
  ...
  int    *xl_, *yl_, *xh_, *yh_;
  int    *layerMask_;
  char* layerPlaceType_;
  int* layerPlaceX_;
  int* layerPlaceY_;
  char* layerPlaceOrient_;
  int    polysAllocated_;
  int    numPolys_;
  char** polygonNames_;
  int    *polyMinSpacing_;
  ...
  defrPropsArray* layerProps_;           // 6.0
  defrPropsArray* polyProps_;            // 6.0
  defrPropsArray* viaProps_;             // 6.0
...
public:
  ...
  void addAPinMaxCutCar(int value, const char* layer);
  void addVia(const char* via, int viaX, int viaY, int color = 0, int orient = 0);      // 5.7
  void addLayerProp(defiProp* prop);                                                    // 6.0
  void addPolyProp(defiProp* prop);                                                     // 6.0
  void addViaProp(defiProp* prop);                                                      // 6.0
  ...
  void addPortVia(const char* via, int viaX, int viaY, int color = 0, int orient = 0);           // 5.7
  void setPortPlacement(int typ, int x, int y, int orient);       // 5.7 - 5.8
  void setPortLayerPlacement(int typ, int x, int y, int orient);
  void setPortPolygonPlacement(int typ, int x, int y, int orient);
  void addProp(defiProp* prop);                                   // 6.0
  void addPortLayerProp(defiProp* prop);                          // 6.0
  void addPortPolyProp(defiProp* prop);                           // 6.0
  void addPortViaProp(defiProp* prop);                            // 6.0
  ...
  int hasLayerPlacement(int index) const;             // per-layer placement
  int layerPlacementType(int index) const;            // per-layer placement type
  int layerPlacementX(int index) const;               // per-layer X
  int layerPlacementY(int index) const;               // per-layer Y
  int layerPlacementOrient(int index) const;          // per-layer orient
  const char* layerPlacementOrientStr(int index) const;  // per-layer orient string
  ...
  int viaOrient(int idx) const;                       // 6.0
  ...
  //6.0
  int   numProps() const;
  const defiProp* prop(int idx) const;
  int   numLayerProps(int layerIdx) const;
  int   numPolyProps(int polydx) const;
  int   numViaProps(int viaIdx) const;
  const defiProp* layerProp(int layerIdx, int propIdx) const;
...
protected:
  ...
  int numLayers_;                   // 5.6
  int *layerMask_;                  // 5.8
  char* layerPlaceType_;            // per-layer placement type
  int* layerPlaceX_;                // per-layer placement X
  int* layerPlaceY_;                // per-layer placement Y
  char* layerPlaceOrient_;          // per-layer placement orient
  ...
  int *polyMask_;                   // 5.8
  char* polyPlaceType_;            // per-polygon placement type
  int* polyPlaceX_;                // per-polygon placement X
  int* polyPlaceY_;                // per-polygon placement Y
  char* polyPlaceOrient_;          // per-polygon placement orient
  int numPolys_;                    // 5.6
  int polysAllocated_;              // 5.6
  defiPoints** polygons_;    // 5.6 
  char lastAddedType_;              // 0=NONE, 1=LAYER, 2=POLYGON
  int x_, y_;            // placement
  int hasSpecial_;
  int numVias_;                     // 5.7
