...
void defiPinPort::Init() {
  ...
  delete layerProps_;
  layerProps_ = new defrPropsArray();         // 6.0

  delete polyProps_;
  polyProps_ = new defrPropsArray();          // 6.0
  
  delete viaProps_;
  viaProps_ = new defrPropsArray();           // 6.0
}
...
void defiPinPort::clear() {
  ...
  if (layers_) {
    ...
    free((char*)(layerPlaceType_));
    free((char*)(layerPlaceX_));
    free((char*)(layerPlaceY_));
    free((char*)(layerPlaceOrient_));
  }
  layers_ = 0;
  layerMinSpacing_ = 0;
  layerEffectiveWidth_ = 0;
  ...
  if (polygonNames_) {
    ...
    free((char*)(polyPlaceType_));
    free((char*)(polyPlaceX_));
    free((char*)(polyPlaceY_));
    free((char*)(polyPlaceOrient_));
    polygonNames_ = 0;
    polygons_ = 0;
    polyMinSpacing_ = 0;
    polyEffectiveWidth_ = 0;
...
void defiPinPort::addLayer(const char* layer) {
  if (numLayers_ >= layersAllocated_) {
    ...
    nlpt = (char*)malloc(layersAllocated_ * sizeof(char));
    nlpx = (int*)malloc(layersAllocated_ * sizeof(int));
    nlpy = (int*)malloc(layersAllocated_ * sizeof(int));
    nlpo = (char*)malloc(layersAllocated_ * sizeof(char));
    ...
    for (i = 0; i < numLayers_; i++) {
       ...
       lm[i] = layerMask_[i];
       nlpt[i] = layerPlaceType_[i];
       nlpx[i] = layerPlaceX_[i];
       nlpy[i] = layerPlaceY_[i];
       nlpo[i] = layerPlaceOrient_[i];
    }
    if (numLayers_ > 0) {
       free((char*)layers_);
       free((char*)xl_);
    ...
    layerMask_ = lm;
    layerPlaceType_ = nlpt;
    layerPlaceX_ = nlpx;
    layerPlaceY_ = nlpy;
    layerPlaceOrient_ = nlpo;
  }
  ...
  layerPlaceType_[numLayers_] = 0;
  layerPlaceX_[numLayers_] = 0;
  layerPlaceY_[numLayers_] = 0;
  layerPlaceOrient_[numLayers_] = 0;
  lastAddedType_ = 1;
  numLayers_ += 1;
  layerProps_->push_back(new defrProps());
}
...
void defiPinPort::addPolygon(const char* layerName) {
  ...
  if (numPolys_ == polysAllocated_) {
    ...
    ppt = (char*)malloc(polysAllocated_ * sizeof(char));
    ppx = (int*)malloc(polysAllocated_ * sizeof(int));
    ppy = (int*)malloc(polysAllocated_ * sizeof(int));
    ppo = (char*)malloc(polysAllocated_ * sizeof(char));
    ...
    for (i = 0; i < numPolys_; i++) {
      ...
      pm[i] = polyMask_[i];
      ppt[i] = polyPlaceType_[i];
      ppx[i] = polyPlaceX_[i];
      ppy[i] = polyPlaceY_[i];
      ppo[i] = polyPlaceOrient_[i];
    }
    if (numPolys_ > 0) {
      free((char*)(polygons_));
      free((char*)(polygonNames_));
  ...
  polyPlaceType_[numPolys_] = 0;
  polyPlaceX_[numPolys_] = 0;
  polyPlaceY_[numPolys_] = 0;
  polyPlaceOrient_[numPolys_] = 0;
  lastAddedType_ = 2;
  numPolys_ += 1;
  polyProps_->push_back(new defrProps());
}
...
void defiPinPort::addVia(const char* viaName, int ptX, int ptY, int color, int orient) {
  if (numVias_ >= viasAllocated_) {
    ...
    no = (int*)malloc(viasAllocated_ * sizeof(int));
  ...
  viaOrient_[numVias_] = orient;

  numVias_ += 1;
  
  if (viaProps_->size() < (size_t) numVias_) {
      viaProps_->push_back(new defrProps());
  }
}
...
void defiPinPort::setPlacement(int typ, int x, int y, int orient) {
  ...
  for (i = 0; i < numLayers_; i++) {
    if (layerPlaceType_[i] == 0) {
      layerPlaceType_[i] = typ;
      layerPlaceX_[i] = x;
      layerPlaceY_[i] = y;
      layerPlaceOrient_[i] = orient;
    }
  }
  
  for (i = 0; i < numPolys_; i++) {
    if (polyPlaceType_[i] == 0) {
      polyPlaceType_[i] = typ;
      polyPlaceX_[i] = x;
      polyPlaceY_[i] = y;
      polyPlaceOrient_[i] = orient;
    }
  }
}

void defiPinPort::setLayerPlacement(int typ, int x, int y, int orient) {
  if (numLayers_ > 0) {
    layerPlaceType_[numLayers_-1] = typ;
    layerPlaceX_[numLayers_-1] = x;
    layerPlaceY_[numLayers_-1] = y;
    layerPlaceOrient_[numLayers_-1] = orient;
  }
}

void defiPinPort::setPolygonPlacement(int typ, int x, int y, int orient) {
  if (numPolys_ > 0) {
    polyPlaceType_[numPolys_-1] = typ;
    polyPlaceX_[numPolys_-1] = x;
    polyPlaceY_[numPolys_-1] = y;
    polyPlaceOrient_[numPolys_-1] = orient;
  }
}

void defiPinPort::addLayerProp(defiProp* prop)
{
    (*layerProps_)[(size_t) numLayers_ - 1]->push_back(prop);
}

void defiPinPort::addPolyProp(defiProp* prop)
{
    (*polyProps_)[(size_t) numPolys_ - 1]->push_back(prop);
}

void defiPinPort::addViaProp(defiProp* prop)
{
    if (viaProps_->size() <= (size_t) numVias_) {
        viaProps_->push_back(new defrProps());
    }

    (*viaProps_)[numVias_]->push_back(prop);
}
...
int defiPinPort::hasLayerPlacement(int index) const {
  if (index < 0 || index >= numLayers_)
    return 0;
  return layerPlaceType_[index] != 0;
}

int defiPinPort::layerPlacementType(int index) const {
  return layerPlaceType_[index];
}

int defiPinPort::layerPlacementX(int index) const {
  return layerPlaceX_[index];
}

int defiPinPort::layerPlacementY(int index) const {
  return layerPlaceY_[index];
}

int defiPinPort::layerPlacementOrient(int index) const {
  return layerPlaceOrient_[index];
}

const char* defiPinPort::layerPlacementOrientStr(int index) const {
  return defiOrientStr(layerPlaceOrient_[index]);
}
...
int defiPinPort::hasPolygonPlacement(int index) const {
  if (index < 0 || index >= numPolys_)
    return 0;
  return polyPlaceType_[index] != 0;
}

int defiPinPort::polygonPlacementType(int index) const {
  return polyPlaceType_[index];
}

int defiPinPort::polygonPlacementX(int index) const {
  return polyPlaceX_[index];
}

int defiPinPort::polygonPlacementY(int index) const {
  return polyPlaceY_[index];
}

int defiPinPort::polygonPlacementOrient(int index) const {
  return polyPlaceOrient_[index];
}

const char* defiPinPort::polygonPlacementOrientStr(int index) const {
  return defiOrientStr(polyPlaceOrient_[index]);
}
...
int defiPinPort::viaOrient(int index) const {
    return viaOrient_[index];
}
...
int defiPinPort::numLayerProps(int layerIdx) const
{
    return (int) ((*layerProps_)[layerIdx]->size());
}

int defiPinPort::numPolyProps(int polyIdx) const
{
    return (int)((*polyProps_)[polyIdx]->size());
}

int defiPinPort::numViaProps(int viaIdx) const
{
    return (int)((*viaProps_)[viaIdx]->size());
}

const defiProp* defiPinPort::layerProp(int layerIdx, int propIdx) const
{
    return (*(*layerProps_)[layerIdx])[propIdx];
}

const defiProp* defiPinPort::polyProp(int polyIdx, int propIdx) const
{
    return (*(*polyProps_)[polyIdx])[propIdx];
}

const defiProp* defiPinPort::viaProp(int viaIdx, int propIdx) const
{
    return (*(*viaProps_)[viaIdx])[propIdx];
}
...
void defiPin::Init() {
  ...
  viaOrient_ = 0;                             // 6.0
  layerPlaceType_ = 0;                       // per-layer placement
  layerPlaceX_ = 0;                          // per-layer placement
  layerPlaceY_ = 0;                          // per-layer placement
  layerPlaceOrient_ = 0;                     // per-layer placement
  polyPlaceType_ = 0;                       // per-polygon placement
  polyPlaceX_ = 0;                          // per-polygon placement
  polyPlaceY_ = 0;                          // per-polygon placement
  polyPlaceOrient_ = 0;                     // per-polygon placement
  props_ = new defrProps();                   // 6.0
  layerProps_ = new defrPropsArray();         // 6.0
  polyProps_ = new defrPropsArray();          // 6.0
  viaProps_ = new defrPropsArray();           // 6.0
}
...
void defiPin::clear() {
  ...
  delete layerProps_;
  layerProps_ = new defrPropsArray();

  delete polyProps_;
  polyProps_ = new defrPropsArray();
  
  delete viaProps_;
  viaProps_ = new defrPropsArray();
...
void defiPin::setLayerPlacement(int typ, int x, int y, int orient) {
  if (numLayers_ > 0) {
    layerPlaceType_[numLayers_-1] = typ;
    layerPlaceX_[numLayers_-1] = x;
    layerPlaceY_[numLayers_-1] = y;
...
void defiPin::setPolygonPlacement(int typ, int x, int y, int orient) {
  if (numPolys_ > 0) {
    polyPlaceType_[numPolys_-1] = typ;
    polyPlaceX_[numPolys_-1] = x;
    polyPlaceY_[numPolys_-1] = y;
...
int defiPin::hasLayerPlacement(int index) const {
  if (index < 0 || index >= numLayers_)
    return 0;
  return layerPlaceType_[index] != 0;
}

int defiPin::layerPlacementType(int index) const {
  return layerPlaceType_[index];
}

int defiPin::layerPlacementX(int index) const {
  return layerPlaceX_[index];
}

int defiPin::layerPlacementY(int index) const {
  return layerPlaceY_[index];
}

int defiPin::layerPlacementOrient(int index) const {
  return layerPlaceOrient_[index];
}

const char* defiPin::layerPlacementOrientStr(int index) const {
  return defiOrientStr(layerPlaceOrient_[index]);
}

int defiPin::hasPolygonPlacement(int index) const {
  if (index < 0 || index >= numPolys_)
    return 0;
  return polyPlaceType_[index] != 0;
}

int defiPin::polygonPlacementType(int index) const {
  return polyPlaceType_[index];
}

int defiPin::polygonPlacementX(int index) const {
  return polyPlaceX_[index];
}

int defiPin::polygonPlacementY(int index) const {
  return polyPlaceY_[index];
}

int defiPin::polygonPlacementOrient(int index) const {
  return polyPlaceOrient_[index];
}

const char* defiPin::polygonPlacementOrientStr(int index) const {
  return defiOrientStr(polyPlaceOrient_[index]);
}
...
// 5.7
void defiPin::addVia(const char* viaName, int ptX, int ptY, int color, int orient) {
...
// 6.0
void defiPin::addLayerProp(defiProp* prop)
{
    (*layerProps_)[(size_t)numLayers_ - 1]->push_back(prop);
}

//6.0
void defiPin::addPolyProp(defiProp* prop)
{
    (*polyProps_)[(size_t)numPolys_ - 1]->push_back(prop);
}

//6.0
void defiPin::addViaProp(defiProp* prop)
{
    if (viaProps_->size() <= (size_t)numVias_) {
        viaProps_->push_back(new defrProps());
    }
...
int defiPin::viaOrient(int idx) const {
    return viaOrient_[idx];
}
...
// 5.7
void defiPin::addPortVia(const char* via, int viaX, int viaY, int color, int orient) {
  int i = numPorts_ - 1;
  pinPort_[i]->addVia(via, viaX, viaY, color, orient);
}
...
void defiPin::setPortLayerPlacement(int typ, int x, int y, int orient) {
  int i = numPorts_ - 1;
  pinPort_[i]->setLayerPlacement(typ, x, y, orient);
}

void defiPin::setPortPolygonPlacement(int typ, int x, int y, int orient) {
  int i = numPorts_ - 1;
  pinPort_[i]->setPolygonPlacement(typ, x, y, orient);
}

void defiPin::addProp(defiProp* prop)
{
    props_->push_back(prop);
}

void defiPin::addPortLayerProp(defiProp* prop)
{
    int i = numPorts_ - 1;
    pinPort_[i]->addLayerProp(prop);
}

void defiPin::addPortPolyProp(defiProp* prop)
{
    int i = numPorts_ - 1;
    pinPort_[i]->addPolyProp(prop);
}

void defiPin::addPortViaProp(defiProp* prop)
{
    int i = numPorts_ - 1;
    pinPort_[i]->addViaProp(prop);
}
...
// 6.0
int defiPin::numProps() const
{
    return (int) props_->size();
}

// 6.0
const defiProp* defiPin::prop(int idx) const
{
    return (*props_)[idx];
}

// 6.0
int defiPin::numLayerProps(int layerIdx) const
{
    return (int)((*layerProps_)[layerIdx]->size());
}

// 6.0
int defiPin::numPolyProps(int polyIdx) const
{
    return (int)((*polyProps_)[polyIdx]->size());
}

// 6.0
int defiPin::numViaProps(int viaIdx) const
{
    return (int)((*viaProps_)[viaIdx]->size());
}

// 6.0
const defiProp* defiPin::layerProp(int layerIdx, int propIdx) const
{
    return (*(*layerProps_)[layerIdx])[propIdx];
}

// 6.0
const defiProp* defiPin::polyProp(int polyIdx, int propIdx) const
{
    return (*(*polyProps_)[polyIdx])[propIdx];
}

// 6.0
const defiProp* defiPin::viaProp(int viaIdx, int propIdx) const
{
    return (*(*viaProps_)[viaIdx])[propIdx];
}
