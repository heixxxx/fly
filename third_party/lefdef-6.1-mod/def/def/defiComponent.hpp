...
#define DEFI_COMPONENT_SOFTFIXED    5
...
public:
  ...
  void addPinprop(const char* pinName, defiProp* pinprop);  // 6.0
  ...
  int    numPinprops() const;                       // 6.0
  const char* pinpropPin(int index) const;          // 6.0
  const defiProp*   pinpropProp(int index) const;   // 6.0
  int    hasPhysical() const;                       // 6.0
