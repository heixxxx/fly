...
void defiRow::addProperty(const char* name, const char* value, char type) 
{
  int len;
...
void defiRow::addNumProperty(const char* name, double d,
...
void defiTrack::setNdr(const char* ndr)
{
    if (!ndr_) {
        ndr_ = new defrString();
    }
...
void defiTrack::setWidth(double width)
{
    hasWidth_ = 1;
    width_ = width;
}

void defiTrack::addProp(defiProp* prop)
{
    if (!props_) {
...
// 6.0
const char* defiTrack::ndr() const {
    if (!ndr_) {
        return NULL;
    }
...
// 6.0
int defiTrack::hasWidth() const {
    return hasWidth_;
}

// 6.0
double defiTrack::width() const {
    return width_;
}

// 6.0
int defiTrack::numProps() const {
    if (!props_) {
        return 0;
    }
...
// 6.0
defiProp* defiTrack::prop(int index)
{
    return (*props_)[index];
}
