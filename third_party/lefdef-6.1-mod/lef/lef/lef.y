...
// ************************************************************************** 
// ************************************************************************** 
// Copyright 2012 - 2021, Cadence Design Systems                                     
//                                                                            
...
%token K_TOPOFSTACKONLY K_PORTOBS
...
%token K_ANTENNAAREAMINUSDIFF K_NOROUTE K_ABSTRACT
...
%token K_EXCEPTPGNET K_OBSSPACING K_FULLDRC K_MIN
...
%type <string> one_pin_trigger req_layer_name prop_string_value
...
%type <string> real_abstract_noroute
...
rule:  version | busbitchars | case_sensitivity | units_section
    ...
    | nondefault_rule | prop_def_section
...
layer_option:
  K_MANUFACTURINGGRID int_number ';'   // 6.0
    {
      if (lefData->versionNum < 6.0 - 0.00001) {
        if (lefData->lef60NewSyntaxError("LAYER ... MANUFACTORINGGRID value ;")) {
          ...
          lefData->lefrLayer.setManufacturingGrid($2);
      }
    }
  | K_ARRAYSPACING                   // 5.7 
    {
...
usevia: 
    ...
    T_STRING ';'
    {
       if (lefData->versionNum < 5.6) {
          ...
          if (lefCallbacks->NonDefaultCbk)
             lefData->lefrNonDefault.addUseVia($3);
       }
    }
...
useviarule: 
    ...
    T_STRING ';'
    {
       if (lefData->versionNum < 5.6) {
          ...
          if (lefCallbacks->NonDefaultCbk)
             lefData->lefrNonDefault.addUseViaRule($3);
       }
    }
...
mincuts: 
    ...
    T_STRING int_number ';'
    {
       if (lefData->versionNum < 5.6) {
          ...
          if (lefCallbacks->NonDefaultCbk)
             lefData->lefrNonDefault.addMinCuts($3, (int)$4);
       }
    }
...
site_prop: K_PROPERTY prop_name_value ';'
  {
    if (lefData->versionNum < 6.0 - 0.00001) {
        ...
        lefData->setPropDataType($2, lefSettings->lefProps.lefrSiteProp);
        lefData->lefrSite.addProp($2);
        $2 = NULL;
    }
...
macro_obsspacing: K_OBSSPACING obsspacing_opt obsspaicing_layers ';'
  {
      if (lefData->versionNum < 6.0 - 0.00001) {
...
obsspacing_opt:
  ...
  | NUMBER
    {
        if (lefCallbacks->MacroCbk) { 
            lefData->lefrMacro.addOBSSpacing("", $1); 
        }
    }

obsspaicing_layers: 
    | obsspaicing_layer obsspaicing_layers
    ;

obsspaicing_layer: K_LAYER {lefData->lefDumbMode = 1;} T_STRING
    {
        lefData->lefrMacro.addOBSSpacingLayer($3);
    }
...
macro_pin_option:
  ...
  | K_TAPERRULE { lefData->lefDumbMode = 1;} T_STRING ';'
    { if (lefCallbacks->PinCbk) lefData->lefrPin.setTaperRule($3); }
...
geometry:
  ...
  opt_geometry_props
  layer_exceptpgnet
  layer_real_abstract_noroute_opt
  layer_spacing ';'
  | K_WIDTH int_number ';'
    { 
      if (lefData->lefrDoGeometries) {
    ...
    | real_abstract_noroute
    {
        if (!lefData->obsDef) {
           ...
           CHKERR();
            ...
            if (lefData->lefrDoGeometries) {
                lefData->lefrGeometriesPtr->addObsType($1);
            }
        }
    }
...
geometry_prop: K_PROPERTY prop_name_value
    {
      if (lefData->lefrDoGeometries) {
        lefData->setPropDataType($2, lefSettings->lefProps.lefrPortobsProp);
        lefData->lefrGeometriesPtr->addProp($2);
        $2 = NULL;
      }
...
opt_geometry_via_props:
  | geometry_via_props
  {}

geometry_via_props: geometry_via_prop
  | geometry_via_props geometry_via_prop
  {}

geometry_via_prop: K_PROPERTY prop_name_value 
  {
    if (lefData->versionNum < 6.0 - 0.00001) {
        ...
        if (lefData->lefrDoGeometries) {
            lefData->setPropDataType($2, lefSettings->lefProps.lefrPortobsProp);
            lefData->lefrGeometriesPtr->addViaProp($2);  
            $2 = NULL;
        }
    }
...
via_placement:
  K_VIA opt_geometry_via_props maskColor pt {lefData->lefDumbMode = 1;} T_STRING ';'
    { 
        if (lefData->lefrDoGeometries){
            if (lefData->versionNum < 5.8 && (int)$3 > 0) {
              if (lefData->macroWarnings++ < lefSettings->MacroWarnings) {
                ...
                lefData->lefrGeometriesPtr->addVia((int)$3, $4.x, $4.y, $6);
            }
        }
    }
...
prop_stmt:
  ...
  | K_PORTOBS {lefData->lefDumbMode = 1; lefData->lefrProp.clear(); }
    T_STRING prop_define ';'
    { 
      if (lefData->versionNum < 6.0 - 0.00001) {
      ...
      if (lefCallbacks->PropCbk) {
        lefData->lefrProp.setPropType("portobs", $3);
        CALLBACK(lefCallbacks->PropCbk, lefrPropCbkType, &lefData->lefrProp);
      }
      lefSettings->lefProps.lefrPortobsProp.setPropType($3, lefData->lefPropDefType);
    }
  | K_SITE {lefData->lefDumbMode = 1; lefData->lefrProp.clear(); }
    T_STRING prop_define ';'
    { 
      if (lefData->versionNum < 6.0 - 0.00001) {
      ...
      if (lefCallbacks->PropCbk) {
        lefData->lefrProp.setPropType("site", $3);
        CALLBACK(lefCallbacks->PropCbk, lefrPropCbkType, &lefData->lefrProp);
      }
      lefSettings->lefProps.lefrSiteProp.setPropType($3, lefData->lefPropDefType);
    }
prop_define:
