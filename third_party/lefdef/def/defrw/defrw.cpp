  ...
  if (ppath->numWires()) {
     ...
     for (i = 0; i < ppath->numWires(); i++) {
        ...
        for (j = 0; j < wire->numPaths(); j++) {
           ...
           for (int k = 0; k < p->getNumElements(); k++) {
              count++;
              // Don't want the line to be too long
              if (count >= 5) {
                  fprintf(fout, "\n");
              ...
              switch (p->getType(k)) {
                case DEFIPATH_LAYER:
                     if (newLayer == 0) {
                         fprintf(fout, "%s ", p->getLayer(k));
                         newLayer = 1;
                     } else
                         fprintf(fout, "NEW %s ", p->getLayer(k));
                     break;
                case DEFIPATH_VIA:
                     fprintf(fout, "%s ", ignoreViaNames ? "XXX" : p->getVia(k));
                     break;
                case DEFIPATH_VIAROTATION:
                     fprintf(fout, "%s ",
                             orientStr(p->getViaRotation(k)));
                     break;
                case DEFIPATH_VIADATA:
                     p->getViaData(k, &numX, &numY, &stepX, &stepY);
                     fprintf(fout, "DO %d BY %d STEP %d %d ", numX, numY,
                             stepX, stepY);
                     break;
                case DEFIPATH_WIDTH:
                     fprintf(fout, "%d ", p->getWidth(k));
                     break;
		case DEFIPATH_MASK:
		     fprintf(fout, "MASK %d ", p->getMask(k));
		     break;
                case DEFIPATH_VIAMASK:
                     fprintf(fout, "MASK %d%d%d ", 
                             p->getViaTopMask(k), 
                             p->getViaCutMask(k),
                             p->getViaBottomMask(k));
                    break;
                case DEFIPATH_POINT:
                     p->getPoint(k, &x, &y);
                     fprintf(fout, "( %d %d ) ", x, y);
                ...
                case DEFIPATH_TAPER:
                     ...
                     break;
                case DEFIPATH_SHAPE:
                     fprintf(fout, "+ SHAPE %s ", p->getShape(k));
                     break;
                case DEFIPATH_STYLE:
                     fprintf(fout, "+ STYLE %d ", p->getStyle(k));
                     break;
              }
           }
        }
