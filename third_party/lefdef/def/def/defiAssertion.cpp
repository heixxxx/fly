...
void defiAssertion::addNet(const char* name) {
  ...
  i = (int) strlen(name) + 1;
  if (name[i-2] == ',') {
     s  = (char*)malloc((size_t)i-1);
     s1 = (char*)malloc((size_t)i-1);
     memcpy(s1, name, (size_t)i-2);
     s1[i-2] = '\0';
