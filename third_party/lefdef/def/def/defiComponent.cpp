...
struct defrPinProp {
    defrPinProp(const char* pinName, defiProp* prop)
        : pinName_(pinName),
        prop_(prop)
    {}
...
class  defrPinProps : public std::vector<defrPinProp>
{
public:
    ~defrPinProps() {
        for (size_t idx = 0; idx < this->size(); idx++) {
            delete (*this)[idx].prop_;
        }
    }
};
...
void defiComponent::IdAndName(const char* id, const char* name) {
  ...
  if ((len = (int) strlen(id)+1) > idSize_)
    bumpId(len);
  strcpy(id_, defData->DEFCASE(id));

  if ((len = (int) strlen(name)+1) > nameSize_)
    bumpName(len);
...
void defiComponent::setRegionName(const char* name) {
  ...
  if ((len = (int) strlen(name)+1) > regionNameSize_)
    bumpRegionName(len);
...
void defiComponent::setEEQ(const char* name) {
  ...
  if ((len = (int) strlen(name)+1) > EEQSize_)
    bumpEEQ(len);
...
void defiComponent::setRouteHalo(int haloDist, const char* minLayer,
  ...
  haloDist_ = haloDist;
  if ((len = (int) strlen(minLayer)+1) > minLayerSize_)
    bumpMinLayer(len);
  strcpy(minLayer_, defData->DEFCASE(minLayer));
  if ((len = (int) strlen(maxLayer)+1) > maxLayerSize_)
    bumpMaxLayer(len);
...
void defiComponent::setPhysical()
{
    hasPhysical_ = 1;
}
...
int defiComponent::isSoftfixed() const
{
  return status_ == DEFI_COMPONENT_SOFTFIXED ? 1 : 0;
}
...
int defiComponent::numPinprops() const
{
    if (!pinprops) {
        return 0;
    }

    return (int) pinprops->size();
}

const char* defiComponent::pinpropPin(int index) const
{
    return (*pinprops)[index].pinName_.c_str();
}

const defiProp* defiComponent::pinpropProp(int index) const
{
    return (*pinprops)[index].prop_;
}

int defiComponent::hasPhysical() const
{
    return hasPhysical_;
}
...
void defiComponent::addNumProperty(const char* name, double d,
...
void defiComponent::addPinprop(const char*   pinName, 
                               defiProp*     pinprop)
{
    if (!pinprops) {
        pinprops = new defrPinProps();
    }

    pinprops->push_back(defrPinProp(pinName, pinprop));
}


void defiComponent::addNet(const char* net) {
  int len = (int) strlen(net) + 1;
...
void defiComponent::setForeignName(const char* name) {
  ...
  if ((len = (int) strlen(name)+1) > foreignNameSize_)
    bumpForeignName(len);
