...
lefReloadBuffer()
{
    ...
    if (lefData->first_buffer) {
        ...
        if (lefSettings->ReadFunction) {
            if ((nb = (int) (*lefSettings->ReadFunction)(lefData->lefrFile, lefData->current_buffer, 4)) != 4) {
                lefData->next = NULL;
                return;
            }
        } else {
            if ((nb = (int) fread(lefData->current_buffer, 1, 4, lefData->lefrFile)) != 4) {
                lefData->next = NULL;
                return;
            }
        }
    ...
    if (lefData->encrypted) {
        ...
        if (lefSettings->ReadFunction)
            nb = (int) (*lefSettings->ReadFunction)(lefData->lefrFile, lefData->current_buffer, IN_BUF_SIZE);
        else
            // This is a normal file so just read some bytes. 
            nb = (int) fread(lefData->current_buffer, 1, IN_BUF_SIZE, lefData->lefrFile);
    }
...
lefsublex()
{
    ...
    /* First, we eat all the things the parser should be unaware og comments
     */
            ...
            if (K_HISTORY == result) {  // history - get up to ';' 
                ...
                if (foundTag) {
                    ...
                    for (cc = lefGetc(); ; cc = lefGetc()) {
                        ...
                        lefData->Hist_text.push_back(cc);     
                        int histTextSize = (int) lefData->Hist_text.size();

                        if (histTextSize >= 6 && memcmp(&lefData->Hist_text[(size_t) histTextSize - 6 ], "ENDEXT", 6) == 0) { 
                            if (begQuote)
                                lefError(1006, "Ending \" is missing");
                            break;
                        } else if (histTextSize >= 11 && memcmp(&lefData->Hist_text[(size_t) histTextSize - 11 ], "END LIBRARY", 11) == 0) {
...
lefMalloc(size_t lef_size)
{
    ...
    if (lefSettings->MallocFunction)
        return (*lefSettings->MallocFunction)((int) lef_size);
    else {
           ...
           size_t  lef_size)
{
    if (lefSettings->ReallocFunction)
        return (*lefSettings->ReallocFunction)(name, (int) lef_size);
    else
