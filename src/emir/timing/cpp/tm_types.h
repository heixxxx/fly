...
inline constexpr uint32_t kTMNoClock = UINT32_MAX;
...
public:
    ...
    TMRange rise_slew_;
    TMRange fall_slew_;
    // 主时钟（首现分组；NULL 源 = kTMNoClock）
    uint32_t clock_id_ = kTMNoClock;
...
public:
    ...
    CMUnorderedMap<CMString, uint32_t> clock_index_;
    ...
    FLY_SERIALIZE(design_, version_, source_file_, time_scale_sec_, vth_low_,
                  vth_high_, default_input_slew_, clocks_, entries_,
                  dropped_source_res_count_, dropped_slack_count_,
                  unknown_construct_count_, bad_record_count_,
