...
def test_all_files_failed_fatals_exit_80():
    ...
    script = (
        ...
        "EXTMFileBinding, tm_plan_file_chunks, tm_convert_chunk)\n"
        ...
        "binding.kind = 0\n"
        "slice_obj = tm_convert_chunk(ctx, path, fp.chunk_starts[0], "
        ...
        "        return plan if k.endswith('plan') else (slice_obj if "
        "'chunk' in k else 0)\n"
        ...
        "['chunk_0_0'], ['conflicts_0'], 'plan', 'clocks', 'summary')\n"
    )
    env = dict(os.environ)
