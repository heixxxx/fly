...
//   - S1 ds_parse_tech_lef：确立全局 DBU 基准 + 层堆叠（Stack 独立类型，
//     裁定 ⑭）+ tech 级 via（权威表首批，裁定 ⑪）+ VIARULE GENERATE 按
//     规则默认参数直接展开为模板 DSViaCell（㉚：DSViaRule 已删除）。
//   - S2 ds_parse_cell_lef：macro/pin/OBS → 临时 DSDesign（局部 id，
//     裁定 ①——全局 id 由 T6 汇总统一重排）+ pin 几何独立对象
//     （⑰：几何不进简化 pin；R4 起按局部平铺 pin id 落位）+ 文件级 via 集。
//
// 初始化序列（T2 实施记录红线）：lefrInitSession() +
// lefrSetRegisterUnusedCallbacks()，缺一会在解析中段段错误（实测）。
//
// 异常语义（dev-rules §7 仅两类可 raise + D15）：
//   文件不可读 / 语法错误（lefrRead 非 0）/ lef 间 DBU 不一致 /
//   层引用未定义（via/pin/obs 引用了 stack 中不存在的层）→
//   std::runtime_error。implant 等非布线/切割层跳过并计数（兜底模式
//   「跳过并计数」），重名 macro/via 保留首份并计数（「抛弃并提醒」，
//   提醒消息 DSGN 注册后于 T6 接线）。
// =============================================================================
...
// S1：tech lef → DSStack（基准确立 + 层堆叠）+ tech 级 via 集（含
// VIARULE GENERATE 展开的模板 DSViaCell，㉚）。
...
// 局部平铺 pin id）+ 文件级 via 集。stack 为只读基准（DBU 校验 +
// 层名→id 解析）。
