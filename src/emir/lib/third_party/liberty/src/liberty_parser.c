...
{
    int yystate;
    /* fly patch (see src_local/PATCHES.md): reset the group stack depth so a
       previously aborted parse (syntax error) cannot leave a stale stack that
       the next parse would attach groups to (use-after-free SIGSEGV). */
    gsindex = 0;
