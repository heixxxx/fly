// *****************************************************************************
// *****************************************************************************
// Copyright 2012 - 2020, Cadence Design Systems
// 
...
lefiGeometries::clear()
{
    for (int i = 0; i < numItems_; i++) {
        ...
        if (itemType_[i] == lefiGeomPropE) {
            delete (lefiProp*)items_[i];
        } else {
                       ...
                       const char   *name)
{
    ...
    if (viaProps) {
        for (size_t idx = 0; idx < viaProps->size(); idx++) {
            addProp((*viaProps)[idx]);
            (*viaProps)[idx] = NULL;
        }
...
void 
lefiGeometries::addProp(lefiProp* prop)
{
    add(prop, lefiGeomPropE);
}
...
void lefiGeometries::addViaProp(lefiProp* prop)
{
    if (!viaProps) {
        viaProps = new lefrPropArray();
    }

    viaProps->push_back(prop);
}

void lefiGeometries::addObsType(const char* type)
{
    char* c = (char*)lefMalloc(strlen(type) + 1);
    strcpy(c, type);
    add((void*)c, lefiGeomObsTypeE);
}
...
const lefiProp* lefiGeometries::getProp(int index) const
{
    char msg[160];
    if (index < 0 || index >= numItems_) {
        sprintf(msg, "ERROR (LEFPARS-1373): The index number %d given for the geometry PROPERTY is invalid.\nValid index is from 0 to %d", index, numItems_);
        lefiError(0, 1373, msg);
        return 0;
    }

    return (const lefiProp*)(items_[index]);
}
...
const char* lefiGeometries::getObsType(int index) const
{
    return (const char*)(items_[index]);
}
...
lefiSpacing::clone()
{
    ...
    sp->name1Size_ = (int) strlen(name1_) + 1;
    sp->name1_ = (char*) lefMalloc(sp->name1Size_);
    strcpy(sp->name1_, name1_);
    sp->name2Size_ = (int) strlen(name2_) + 1;
...
lefiSpacing::set(const char *name1,
                 ...
                 int        hasStack)
{
    int len = (int) strlen(name1) + 1;
    if (len > name1Size_) {
        lefFree(name1_);
        name1_ = (char*) lefMalloc(len);
        name1Size_ = len;
    }
    len = (int) strlen(name2) + 1;
    if (len > name2Size_) {
        lefFree(name2_);
...
// 6.0
void 
lefiSite::addProp(lefiProp* prop)
{
    if (!props_) {
        props_ = new lefrPropArray();
    }
