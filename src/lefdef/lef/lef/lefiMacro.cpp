...
class lefrOBSSpacing {
public:
    lefrOBSSpacing(const char* type, double spacing = 0.0) 
        : type_(type),
          spacing_(spacing)
    {
    }
...
class lefrOBSSpacings : public std::vector<lefrOBSSpacing> {
};
...
void 
lefiMacro::addOBSSpacing(const char* type, double spacing)
{
    if (!obsSpacings_) {
        obsSpacings_ = new lefrOBSSpacings();
    }

    obsSpacings_->push_back(lefrOBSSpacing(type, spacing));
}

void lefiMacro::addOBSSpacingLayer(const char* layer)
{
    int idx = (int) obsSpacings_->size() - 1;

    (*obsSpacings_)[idx].layers_.push_back(layer);
}
...
int 
lefiMacro::numOBSSpacings() const
{
    if (!obsSpacings_) {
        return 0;
    }

    return (int) obsSpacings_->size();
}

// Returns "FULLDRC" | "MIN" | "".
const char* 
lefiMacro::OBSSpacingType(int idx) const
{
    return (*obsSpacings_)[idx].type_.c_str();
}

double 
lefiMacro::OBSSpacingSpacing(int idx) const
{
    return (*obsSpacings_)[idx].spacing_;
}

int 
lefiMacro::numOBSSpacingLayers(int idx) const
{
    return (int) ((*obsSpacings_)[idx].layers_.size());
}

const char* 
lefiMacro::OBSSpacingLayer(int idx, int layerIdx) const
{
    return (*obsSpacings_)[idx].layers_[layerIdx].c_str();
}
