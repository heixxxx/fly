  ...
  AllowVer60Plus(0)
{
    ...
    // Define LEF58_TYPE (LEF_CDN_TYPE) values and dependences here:
    ...
    addLef58Type("STACKEDMIMCAP", mimcapLayers);
    addLef58Type("TSV", cutOnly);
    addLef58Type("PASSIVATION", cutOnly);
    addLef58Type("HIGHR", cutOnly);
    addLef58Type("SUBSTRATE", cutOnly);
    ...
    addLef58Type("TRIMDIFFUSION", mastersliceOnly);
    addLef58Type("PROBE", mastersliceOnly);
}
...
lefrSettings::init_symbol_table()
{
    ...
    Keyword_set["ABSTRACT"] = K_ABSTRACT;
    ...
    Keyword_set["OBSSPACING"] = K_OBSSPACING;
    Keyword_set["OFF"] = K_OFF;
...
{
    std::string  divChars = " \n\t\r;";
    int          tokenStart = (int) input.find_first_not_of(divChars, 
                                                            startIdx);
    int          tokenEnd = (int) input.find_first_of(divChars, 
                                                      tokenStart);

    startIdx = tokenEnd;
    return input.substr(tokenStart, (size_t)tokenEnd - (size_t)tokenStart);
}
...
lefrSettings::getLayerLef58Types(const char *type) const
{
    ...
    for (; pairsIter != Lef58TypePairs.end(); ++pairsIter) {
        ...
        int     sepIdx = (int) pair.find(' ');

        if (pair.substr((size_t) sepIdx + 1) != type) {
            continue;
        }
