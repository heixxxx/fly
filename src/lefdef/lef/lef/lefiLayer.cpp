...
lefiParallel::addParallelWidth(double width)
{
    if (numWidth_ == numWidthAllocated_) {
        ...
        if (numLength > 0)
            nws = (double*) lefMalloc(sizeof(double) * (size_t) numWidthAllocated_ *
                                                        (size_t) numLength_);
        else {
                            ...
                            int     hasPRL)
{
    ...
    if (numWidth_ == numWidthAllocated_) {
        ...
        if (numWidth_ > 0) {
            nws = (double*) lefMalloc(sizeof(double) * ((size_t) atNsp_[numWidth_ - 1]
                                                        + (size_t) numSpacing));
...
lefiLayer::clear()
{
    ...
    // 6.0
    hasManufacturingGrid_ = 0;
    manufacturingGrid_ = 0;
}
...
{
    if (spacingName) {
        int len = (int) strlen(spacingName) + 1;
...
// 6.0
void 
lefiLayer::setManufacturingGrid(double value)
{
    hasManufacturingGrid_ = 1;
    manufacturingGrid_ = value;
}
...
// 6.0
int 
lefiLayer::hasManufacturingGrid() const
{
    return hasManufacturingGrid_;
}

// 6.0
double 
lefiLayer::manufacturingGrid() const
{
    return manufacturingGrid_;
}
...
// PRIVATE 5.8
// This function will parse LEF58_TYPE (LEF_CDN_TYPE) property value. It also checks
...
lefiLayer::parseLayerType(int index)
{
    ...
    std::string layerTypePropName = (lefData->versionNum < 6.0 - 0.00001) ?
        "LEF58_TYPE" : "LEF_CDN_TYPE";

    // Wrong LEF58_TYPE (LEF_CDN_TYPE) syntax.
    if (firstToken != "TYPE") { 
        std::string msg = "Incorrect " + layerTypePropName + " property value syntax: '" + 
                           propValue + 
    ...
    if (layerLef58Types.empty()) {
        std::string msg = "Layers with TYPE " + 
                           type  + " cannot have " + layerTypePropName + " property.\n";

        lefError(1328, msg.c_str());        
    } else {
        std::string msg = "Property " + layerTypePropName + " has incorrect TYPE value: '" +
                          lef58Type + "'. For TYPE " +
...
// property name is "LEF58_TYPE" (LEF_CDN_TYPE).
void
lefiLayer::parseLEF58Layer()
{
    ...
    for (i = 0; i < numProps_; i++) {
            if (strcmp(names_[i], "LEF58_TYPE") == 0) {
                if (lefData->versionNum < CURRENT_VERSION - 0.00001) {
                    parseLayerType(i);
                } else {
                    lefWarning(2082, 
                                "In DEF 6.0 and newer LEF LAYER 'LEF58_TYPE' property role replaced by 'LEF_CDN_TYPE' property. Rename the property.");
                }
            } else if (strcmp(names_[i], "LEF_CDN_TYPE") == 0) {
                if (lefData->versionNum >= CURRENT_VERSION - 0.00001) {
                    parseLayerType(i);
                }
            }
