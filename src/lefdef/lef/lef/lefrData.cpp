...
int 
lefrData::lef60NewSyntaxError(const char* address)
{
    if (nonDefaultWarnings++ < lefSettings->NonDefaultWarnings) {
        char    *lefMsg = (char*)malloc(1000);

        sprintf(lefMsg,
                "Syntax '%s' is available only in version 6.00 and later. However, your LEF file is defined with version %g.",
                address,
                versionNum);
        lefError(2083, lefMsg);
        free(lefMsg);
...
int 
lefrData::lef60SyntaxError(const char* rule)
{
    if (nonDefaultWarnings++ < lefSettings->NonDefaultWarnings) {
        char    *lefMsg = (char*)malloc(1000);
        
        sprintf(lefMsg,
                "Syntax Error: In LEF version 6.00 and after %s. Translation will be terminated.",
                rule);
        lefError(2084, lefMsg);
        free(lefMsg);
        return 1;
    }
...
int 
lefrData::lef60ObsoltedError(const char* address)
{
    if (nonDefaultWarnings++ < lefSettings->NonDefaultWarnings) {
        char    *lefMsg = (char*)malloc(1000);

        sprintf(lefMsg,
                "Syntax '%s' is obsoleted in version 6.00 and later. However, your LEF file is defined with version %f2.",
                address,
                versionNum);
        lefError(2085, lefMsg);
        free(lefMsg);
...
void 
lefrData::setPropDataType(lefiProp*     prop,
                          lefiPropType& propType)
{
