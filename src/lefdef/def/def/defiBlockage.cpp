...
// 6.0
void defiBlockage::setOnlypgnet() {
    hasOnlypgnet_ = 1;
}

void defiBlockage::setName(const char* name)
{
    if (name_ == 0) {
...
void defiBlockage::addProp(defiProp* prop)
{
    if (!props_) {
...
void defiBlockage::setOnlyblocks()
{
    hasOnlyblocks_ = 1;
}

// 5.7
void defiBlockage::setSoft(double maxDensity) {
  hasSoft_ = 1;
  softMaxDensity_ = maxDensity;
}

// 5.7
void defiBlockage::setPartial(double maxDensity, int noFlops) {
  maxDensity_ = maxDensity;
  partialNoFlops_ = noFlops;
}
...
// 6.0
int defiBlockage::hasOnlypgnet() const {
    return hasOnlypgnet_;
}
...
int defiBlockage::hasName() const
{
    return name_ ? 1 : 0;
}

const char* defiBlockage::name() const
{
    if (name_) {
        return name_->c_str();
    }
...
int defiBlockage::numProps() const
{
    if (!props_) {
        return 0;
    }
...
const defiProp* defiBlockage::prop(int idx) const
{
    if (!props_) {
        return 0;
    }
...
int defiBlockage::hasOnlyblocks() const
{
    return hasOnlyblocks_;
}

double defiBlockage::softMaxDesity() const
{
    return softMaxDensity_;
}

int defiBlockage::hasPartialNoFlops() const
{
    return partialNoFlops_;
}
