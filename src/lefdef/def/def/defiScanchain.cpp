...
// 6.0
void defiOrdered::addProp(defiProp* prop)
{
    if (!props_) {
...
// 6.0
void defiOrdered::setName(const char* name)
{
    delete name_;
    name_ = new defrString();
    name_->assign(name);
}
...
// 6.0
int defiOrdered::numProps() const
{
    if (!props_) {
        return 0;
    }
...
// 6.0
const defiProp* defiOrdered::prop(int index) const
{
    return (*props_)[index];
}

// 6.0
const char* defiOrdered::name() const
{
    if (!name_) {
        return NULL;
    }
...
// 6.0
void defiScanchain::closeOrderedList()
{
    orderedListStarted_ = 0;
}

// 6.0
int defiScanchain::hasOpenedOrderedList()
{
    return orderedListStarted_;
}
...
// 6.0
void defiScanchain::addOrderedProp(defiProp* prop)
{
    ordered_[numOrdered_ - 1]->addProp(prop);
}

// 6.0
void defiScanchain::setOrderedName(const char* name)
{
    ordered_[numOrdered_ - 1]->setName(name);
}


void defiScanchain::startOrderedList() {
  defiOrdered* o;
...
// 6.0
void defiScanchain::addProp(defiProp* prop)
{
    if (!props_) {
        props_ = new defrProps();
    }
...
// 6.0
int defiScanchain::numProps() const
{
    if (!props_) {
        return 0;
    }
...
// 6.0
const defiProp* defiScanchain::prop(int index) const
{
    return (*props_)[index];
}
...
// 6.0
int defiScanchain::numOrderedProps(int index) const
{
    return ordered_[index]->numProps();
}

// 6.0
const defiProp* defiScanchain::orderedProp(int index, int propIdx) const
{
    return ordered_[index]->prop(propIdx);
}

// 6.0
const char* defiScanchain::orderedName(int index)
{
    return ordered_[index]->name();
}
