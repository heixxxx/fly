...
CompressedChunk ZlibCompressor::compress(std::string_view input) {
    CompressedChunk chunk;
    chunk.uncompressed_size_ = static_cast<int32_t>(input.size());
...
int32_t ZlibCompressor::decompress_to(std::string_view compressed_data,
    ...
    if (result != Z_OK) {
        ERR("Zlib decompress_to failed: error {}", result);
        return -1;
    }
