...
//
// merge 跨 path 时（db_path 变化），在源 db_path 根目录写此文件，指向 merge
...
struct MigrationHeader {
    CMString target_db_path_;   // merge 产物 db_path（idx/_DB_META 所在）
    CMString target_data_path_;   // merge 产物 data_path（.dat 所在）
    int64_t migrated_at_ = 0;     // 迁移时间戳（epoch seconds）

    FLY_SERIALIZE(target_db_path_, target_data_path_, migrated_at_)
};
