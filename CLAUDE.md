...
| `object_cache.h` | 两层 LRU 读缓存：low=压缩字节(CMString)，high=反序列化对象(std::any 持 CMSharedPtr<T>)。read_object_compressed 填 low，read_object<T> 填 high；LFU 淘汰 + 30s 保护期 + 1.5× 硬限制 |
