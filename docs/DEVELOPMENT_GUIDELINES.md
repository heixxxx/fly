...
| `_s6_hier_task` | `_build_hier_tree_task`（层级树构建） |
...
落盘对象名 / 分布式文件名（write_object 的 name、产物对象名构造器、
映射段名等一切进入存储层的名字）**仅允许**：

- 大小写字母 `[A-Za-z]`、数字 `[0-9]`
- 符号仅限 `.` 与 `_`

禁止 `/`、`-`、空格及其他任何符号。层级分隔用 `.`（如
`PART_0_0.GEOMETRY`、`id_partition_map.INST.S0`）。理由：文件名跨
文件系统 / 工具链的最大兼容面（路径分隔符、shell 转义、大小写不敏感
文件系统的歧义来源全部排除）。

### 2.10 注释与自解释性（2026-09-17 用户裁定）

**规则**：代码自解释优先。**需要长注释解释「这段代码在做什么、怎么
做」是自解释性差的信号——重构代码（拆函数、命名、结构调整），不是
写注释补丁。**

- 「是什么 / 怎么做」由命名与结构承担：函数名说职责、参数名说含义、
  小函数说步骤——读者不需要注释翻译代码；
- 注释只保留代码**表达不了**的信息，且要短：
  - 为什么这样做（约束、权衡、坑——一两句）；
  - 外部契约引用（裁定/文档/先例的**简短指针**，不在代码内复述全文）；
- **重构信号**：段落注释行数接近或超过所解释的代码行数；注释在逐行
  复述逻辑；读者需要先读注释才能看懂代码——出现任一，先重构再留短注
  释；
- 裁定编号、方案章节、先例形态的叙事属文档内容（docs/ 与
  DOC_CHANGELOG），代码内至多一行指针（如「见 §19」），不复制叙事。

### 2.10 之前的历史段落注释存量按此规则逐步瘦身（随重构批次）。
...
**规则**：设计与编码时，始终以「输入文件数量多、单文件很大（GB 级）」为
常态假设评估方案——小数据下的「够用」结论不得外推到大输入。

**检查清单**（方案评审与自查逐项过）：

1. **编排路径（master/提交线程）只随输入数量线性，不随字节数线性**——读
   文件内容类 IO（嗅探/切块/解析）一律下放 worker 任务并行；master 只做
   参数与元数据级操作（校验/存在性/建库/任务提交）。反例：TWF 切块扫描
   曾在 master 同步读全文件——QA 小文件下「毫秒级」成立，GB 级 × 慢盘
   场景 master 串行 IO 数百秒、worker 全程空转（2026-09-17 发现，已改
   每文件一个 worker 入口任务）。
2. **无全量读入内存的隐含假设**——逐对象/流式/分块处理为默认；确需全量
   载入的要注明规模前提与上限行为。
3. **复杂度对大 N 敏感点显式标注**——O(N²) 及以上（如全量反查、逐条目
   线性扫）在数据规模放大前评估或落演进项。
4. **性能判断必须标注规模前提**——「毫秒级」「可忽略」类结论注明基于的
   输入量级；QA 数据小的验证结论不代表大数据行为。
5. **锁内 IO、同步等待段对规模的敏感性**——持锁期间的工作量、同步段的
   耗时上界不随输入字节增长。

**配套范式**（建库 API 三段式，随重构批次入 dev-rules 细化）：轻量参数
预处理（纯参数/元数据）→ flow 任务提交（每文件一个入口任务，内容 IO 在
worker）→ 顶层 freeze 任务（固定标记对象依赖）。

## 21. db 数据所有权：禁跨 db 数据搬运

（2026-09-18 用户裁定）

**规则**：每个 db 仅写入**属于自己的数据**（本 db 的业务产物、构建元
数据、自身流程的框架临时对象）。**严格禁止把其他 db 的数据单纯搬运到
本 db**——无任何收益的行为。

**论据**（2026-09-18 timing 快照搬运层实证拆除）：
- 「避免下游任务读放大」不成立——搬运后下游任务仍逐任务全量读，只是
  换了 db 读，读取次数与字节数不变；
- 缓存层面无差异——`ObjectCache` 是进程级全局单例（不分 db），直接跨
  db 读与读本 db 副本同样命中；
- 一致性无诉求——上游 db 对象一次性写定，锚定即稳定，无需固化时间点
  快照。

**跨 db 消费的正确形态**：任务以 `as_task`/`wait_obj` 锚**上游 db 对象
全名**（`db.get_full_name(...)`，全局唯一，跨 db 依赖天然支持），任务
体内直接经上游 db 句柄读取——依赖即数据就绪，无中转、无副本。
