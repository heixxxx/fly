...
### P0-3 大对象分片传输 + 背压 ✅（分块/分片已落地 2026-08-29/30；credit 裁定不做）

- **落地状态（2026-09-03 标注）**：帧长 64 位帧头 + 分块分片（DATA_CHUNK 帧流、块 CRC、resend byte-offset）+ 读流式 + 写流式已随 chunked-transfer L0-L3+L1 与 v2 统一块模型全层落地（[chunked-transfer-design.md](chunked-transfer-design.md) §13/§14.7）；credit 窗口流控维持裁定不做。以下为原调研记录。

- **现状**：网络单帧长度为 uint32（≈4GB 硬上限），对象整取不分片，无 credit 流控；并发大读仅受连接池（`data_client_pool_size`）约束。分片+背压在 roadmap F4 被降级。
- **差距**：GB 级矩阵块/波形对象撞单帧上限。
- **调研结论（2026-08-28 代码实证）**——按 fly「内网流程式批处理、move-compute-to-data、远程读为兜底路径」的定位分层裁定：
  - **credit 窗口流控：裁定不做**。credit 防御的是高 QPS 共享链路上「发送速率持续超过接收消费速率」的失衡（慢消费者、多租户竞争）；fly 数据面为每连接同步 request-response（任意时刻单 in-flight）+ 接收方同步解压入缓存（消费即时），发送速率天然耦合在 TCP 拥塞窗口上，独占内网链路无竞争流量——该问题在 fly 的负载画像中不存在。
  - **背压：结构性已含大半，无需新增机制**。server 端 epoll oneshot + 客户端独占 fd → 每连接至多 1 个 in-flight 请求，`send_queue_` 深度上界 = 活跃连接数（nsd×`data_client_pool_size`）；client 端 slot 信号量（默认 4/worker）封顶拉取并发。残余风险仅为慢客户端 HOL——`n_send` 线程之一被大传输 `writev` 占住（10G 内网 GB 级 ≈1s、1G ≈10s），批处理语义下无正确性影响，且 `data_server_threads` 可调。
  - **分块：唯一有长期真实价值的部分，绑定触发条件缓议**。其价值是**内存有界化**而非网络公平——当前整取驻留链 ≈ 2×压缩（server send 队列 pin + client 接收缓冲）+ 1×原始（ObjectCache，受 `read_cache_size` 上限约束），GB 级对象 × 4 并发 ≈ 16GB 峰值，**先撞内存墙而非带宽墙**；分块同时是流式解压落盘的前提。触发条件：单对象压缩后常态超过 worker 内存约 1/8（≈EMIR 亿级子域落地时）；届时与存储层「分块流式读」合并为单一立项（保持绑定关系），不引入独立 credit 协议。
  - **帧长 uint32：正确性缺陷（非容量策略问题），独立先行修复**——超 4GiB 静默回绕 + client 侧 256MB 假上限；方案为 64 位帧头（48 位长度 + 16 位校验位 = 魔数⊕长度折叠，垃圾帧头以 65535/65536 概率当场拒绝，不依赖猜测尺寸上限）。✅ 已落地（2026-08-29 L0：`src/network/cpp/message_protocol.h` 8B 帧头）。
- **分块实现设计调研（2026-08-28）**：磁盘格式与压缩层已具备分块基础（4MB 独立压缩块 + 按需解压 streambuf + FlyStream 双模式），整块边界仅写累积/网络整帧/读整解压三处；分层方案 L1（写直写落盘）/L2（网络分片）/L3（读流式消费）/L4（块级寻址，远期）详见 **[chunked-transfer-design.md](chunked-transfer-design.md)**。
- **建议**：~~维持 F4 ⏸ 降级并更新口径~~ 已被 2026-08-29 立项取代——分块全层落地，credit 流控维持不做。
...
| ~~分块流式读~~ ✅ 已落地（2026-08-30） | 原「读侧对象整取，大对象读放大」——L3 读流式 + DiskChunkSource pread 拉取式已与 P0-3 一体落地（[chunked-transfer-design.md](chunked-transfer-design.md) §14.7 阶段三） | —— |
