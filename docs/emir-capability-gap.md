...
- **调研结论（2026-08-28 代码实证）**——按 fly「内网流程式批处理、move-compute-to-data、远程读为兜底路径」的定位分层裁定：
  ...
  - **帧长 uint32：正确性缺陷（非容量策略问题），独立先行修复**——超 4GiB 静默回绕 + client 侧 256MB 假上限；方案为 64 位帧头（48 位长度 + 16 位校验位 = 魔数⊕长度折叠，垃圾帧头以 65535/65536 概率当场拒绝，不依赖猜测尺寸上限）。已裁定，待实施。
...
- **建议**：维持 F4 ⏸ 降级并更新口径——「分块（绑定亿级触发，与存储层合并立项）；credit 流控裁定不做」。
