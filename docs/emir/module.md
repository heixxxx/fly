...
| `timing/` | ✅ | timing db（⑦ 时序数据库，**全链已落地**——解析器 2026-09-15 立项、建库 flow 2026-09-16 立项实施，方案 [timing-db-plan.md](timing-db-plan.md) §1-13 全裁定）：`build_timing_db(name, timing_files, design_db, settings, alpha)` flow——直接任务链（不用 MapReduce，裁定 2026-09-15）：T1 切块扫描（master 侧单任务同步执行：字节流扫顶层构造括号深度边界切块，alpha `chunk_size_mb` 缺省 256MB）→ design 快照任务（异步——master 提交后立即返回，评审 P2-3；依赖系统只等必要数据对象——层级树/DSBlockNames 全集/id_partition_map INST 段表/pg_nets/global_density[S8 分区表锚]，不等待 design db 冻结；快照组装 + 绑定目标校验后在 worker 上动态提交下游全链——同 design flow S9 plan 任务先例；绑定目标未命中任务内 ValueError §7.5，任务失败语义不阻塞提交）→ T2 逐块解析任务全并行（块自包含 = 头段公共前缀拼块，评审 P1-1；tm_convert_chunk：块重包 `(TIMING_WINDOWS` 外皮解析 + §7 名字换算一次完成——纯路径全层级名（2026-09-16 命名裁定：不含设计名前缀）/ block_inst 块实例绑定局部名偏移换算（§7.1，块端口条目归块实例自身）/ block_cell 定义级建库期复制（§7.2）/ strip_prefix 段级精确剥离（§7.4，u_dutx 防误配）；pin 全局名字空间单哈希查（裁定 3，无 cell 组合键）；网条目锚定全部 driver 位条目（裁定 4 多驱动不取首个）；design db NETS 只收有布线几何的网——未布线设计的网条目全部计 TIMG::0003 悬空（合法兜底口径）；pg 网跳过 §7.5）→ T3 每分区一合并任务（冲突判定按来源文件区分——跨文件同名条目保留首份 TIMG::0006，同文件跨块 NET/PIN 同源形态静默合并）→ T4 汇总（summary 聚合 + 时钟表跨文件合并——顶层文件定义优先、块绑定文件序首份兜底，周期/沿差异 TIMG::0007 计数，裁定 5）→ freeze。单块语法破损兜底（范式 (b) 空分片照常产出）；全部文件失败 TIMG::0009 fatal 码 80（范式 (a)）。消息族 TIMG::0001-0010（register_msg 注册，C++ 计数器经 TMSummary 落库、flow 侧读汇总一次发出）。正式对象 `PART_{xp}_{yp}.TIMING`（TMPartitionTiming primary 恰一——时序点数据无 extend 副本语义）+ `clocks`（TMClockTable——频率 = 1/period 由消费侧推导，Innovus 格式无实例级频率字段，裁定 3）+ `summary`（TMSummary 逐来源文件统计可追溯）+ `alpha_settings`（TMAlphaSettings 两键 chunk_size_mb/format）。id 体系全 CM 族强类型（CMClockId 时钟表下标收编）；消费 design db 快照上下文 TMDesignContext（CMSharedPtr 共享注入，worker 端组装）。读库 API（tm_functions.py 四口 @wait_obj）：load_timing_clocks/load_timing_summary/iter_timing_partition/load_partition_timing + debug get_timing 单实例点查（design db id_partition_map.INST 定位（数据库链 find_db）→ 整区 LRU 容量 8 同 design 先例）。QA：qa/emir/test_emir_timing_flow.py（tm_design 三维度 + 块绑定两形态 + strip_prefix + 时钟合并优先级）+ test_emir_timing_pg_grid.py（pg_grid 12943 条大规模 + 2x1 分区路由，裁定 8 下限断言）。C++ 解析器与测试数据三途径详史见 git 历史（本行 2026-09-16 前版本） |
