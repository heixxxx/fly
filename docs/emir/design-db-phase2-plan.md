...
| R9 | **wait_obj 依赖传播体系（㊼，规范见 DEVELOPMENT_GUIDELINES Section 17）**：框架层——wait_obj wrapper 增 deps(*args) 方法 + run_direct(func,...) 公共 API（经 _fly_original_func 直调）+ 单测（deps 解析/run_direct 零等待）；ds_functions 全部 read_object 类 API wait_obj 包装（含 R7 伴生加载新 API）；ds_flow 任务链调用点改 deps 传播 + run_direct 形态 | R7 |
