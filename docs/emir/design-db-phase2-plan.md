...
| S10 | **汇总校验 + 冻结前置（2026-09-13 校验分级裁定定稿，见 design-db-plan.md S10 节补记 + 实施备注）**：①`ds_verify.h/.cpp`：`DSIdDomain`（expected/actual/holes/duplicates 观测域）+ `DSPartitionCheckResult`（分区级计数 primary/副本/网/跨分区/图形条目/连接 + 全局校验素材 id 集——primary/全量实例/网覆盖（net 0 桶纯 OBS 不计）/crossing 升序去重）+ `DSDesignCheckReport`（损坏类 union_inconsistency_/coverage_gap_/namemap_inconsistency_ 非空即 fatal + 观测类三域 + density_variance_ + 全局统计 + 密度三通道总量）+ `ds_verify_partition`（每分区一调用，只读本分区四类正式产物——分区校验并行不跨区读红线）+ `ds_verify_design`（纯函数：损坏类写报告字段不直接 fatal——处置权在调用方 `ds_verify_report_or_fatal` 逐项 MSG_FATAL_EXIT 码 80，可 fork 单测）+ 校验口径六条（instance expected 扣保留槽与 root 自身/多 primary 直方图超量 = duplicates/网覆盖 net 0 OBS 判别/via id 换算经 via_start + local − 1/密度守恒 primary 口径 = Σ primary vs Σ 首份定义 (实例数 − UNPLACED)/覆盖 = core 行列网格性质 + global_density 覆盖域对齐 int64 域）；②导出面（EXDSIdDomain/EXDSPartitionCheckResult/EXDSDesignCheckReport + 三函数）+ `load_design_verify_report`（R9 wait_obj 形态）+ `VERIFY_REPORT_OBJ = "verify_report"`；③flow：S9 plan 任务动态提交 S10 链（每分区一校验任务并行 + 全局汇总校验任务，merge 之后 freeze 之前）——freeze final_keys 增 verify_report、temp_keys 增校验结果临时对象；损坏类 fatal = report 不落盘 + freeze 依赖缺失（损坏库不冻结，fatal 语义 + 依赖挂接双保险），观测类 warn 不阻断 + 统计 DSGN::0024 INFO；④消息 id 顺延分配：DSGN::0019（FATAL 并查集）/0020（FATAL 分区覆盖）/0021（FATAL namemap）/0022（WARN id 连续性）/0023（WARN 密度守恒）/0024（INFO 统计汇总）；⑤单测 ds_verify_test 22 用例（分区计数与素材/net 0 纯 OBS 不计/覆盖五形态含空设计放行/并查集三形态/namemap 双向/id 空洞重复 warn 不 fatal/primary 守恒过与偏差/统计/序列化往返/fatal fork rc=80 ×3 + 干净不 fatal）+ QA S10 段（test_emir_partition 全干净手算 + test_emir_project_design UNPLACED 空洞触发 0022 观测 warn，0024 统计断言） | S9 merge 产物 + S7/S8 正式对象 + 全部 per-DEF 产物与伴生名 | ✅（2026-09-13 完成） |
| D-DBG | **debug 读库 API + id→partition 反向映射 + net USE 全量补收（2026-09-13 用户裁定定稿，见 design-db-plan.md S5b/S9 节补记与 §6 debug API 表）**：①`DSNetUse` 八值枚举（DEF 规范全集，缺省 SIGNAL）——S5b 解析收 USE 语句（未知值 → SIGNAL 兜底 + unknown_use_count 计数 + DSGN::0026 WARN；defi 语法层校验 USE 值，兜底为防御层）、pg 判定改由 use 枚举派生（special 位保留独立信号源）；分区产物 `DSPartNetConnections` 增 per-net use map（跟随 net 副本、只记非 SIGNAL）；②`ds_id_map.h/.cpp`：分段映射（段粒度 2^20 id 直接索引 pids 数组 + 空洞哨兵 + 段表）+ `DSIdPartitionSlice` 片段（S9 分区合并任务写本区：INST = primary id 集 / NET = geometry 键集）+ `ds_merge_id_partition_slices`（排序线性分段、空洞段跳过、依赖恰一 primary 不变式不重复全空间校验）+ `_id_map_merge_task`（S9 plan 动态提交，段对象 + 段表正式对象，freeze 挂段表）；③DesignDb 六 debug 方法（ds_db.py）：get_cell/get_instance/get_net/get_layer/convert_to_id/convert_to_name——id↔name 转换（inst/net 层级路径 mapper + cell/pin/layer/via_cell hasher）、映射定位分区 + 整分区对象按需加载 + LRU（段/分区容量定值 8）、方向/pg/clock 统计数 flags 位零推导、pg 网不返回 connections 明细、对象名 API 与段常量进 ds_db.py；④导出面：EXDSIdPartitionSlice/Segment/Index + 两函数 + use 字符串化 + pin_key_by_id/via_cell_name_by_id/is_std_cell 补绑；⑤单测：ds_id_map_test 7 用例（提取/merge 分段/空洞段跳过/重复 id 确定性/边界/序列化往返）+ ds_net_pipeline_test USE 三用例（八值解析/未知兜底/字符串化）+ 适配层 use 收录断言 + ds_flatten_test use 跟随副本与往返两用例；⑥QA：test_emir_project_design debug 段（design db 映射段读回 + 六 API 手算）+ debug db 独立 lef/def 数据（data/debug/：INOUT pin cell 构造 hybrid、USE TIEOFF/ANALOG/POWER、pg 网概要、S10 统计手算 connections 12） | S9/S10 全链 + R7 name mapper | ✅（2026-09-13 完成） |
| D-NETRE | **partition 网数据结构重组 + 序列化 set 全族接入（2026-09-13 用户终态裁定，见 design-db-plan.md S9 节「partition 网数据结构重组裁定」）**：①`DSPartitionNets`（NET_CONNECTIONS → NETS 对象更名）：信号网/pg 网分表（`DSNet{net_id, use, connections}` 单网聚合——use 自 S5b `net_uses_` 随网写入；连接条目 `DSNetConnEntry{inst_id, pin_id, flags 六位}`，网 id 由键承载）——信号网全量补全 / pg 网 pg_nets_ 表不补全口径不变；`DSPartNetConnections`（+uses_ 旁挂）删除；②INST_CONNECTIONS 保留（双索引双遍历，条目 `DSPartConnection` 与 DSNetConnEntry 同款对齐 + 端点网 id），四类产物形态不删不改语义；③全局 pg 网 id 集 `"pg_nets"`（`DSPgNetSet` power/ground 两 `CMUnorderedSet` O(1) 判定）——`ds_collect_pg_net_slice`（分区 NETS → pg 片段）+ `DSPgNetSet::finalize_from_flatten`（去重合并）+ `_pg_nets_merge_task`（S9 plan 动态提交，freeze 挂 PG_NETS_OBJ）+ debug API get_net is_pg 经集判定 + `load_design_pg_nets` 查询口；④序列化框架一次性补齐 bitsery v5.2.4 全部类型分派面（`serialization_macros.h`：set 四种 StdSet / optional / variant / tuple + pair / array / list 族 / queue / stack / atomic / chrono / bitset + `fly_ser::elem` 统一元素分派器——pg_nets_、crossing_nets_ 两处「map 充当 set」妥协回归 `CMUnorderedSet` 直存）；⑤S10 适配：`ds_verify_partition` 第四参改 `DSPartitionNets`（两表键与连接条目计数、素材取表键），iconn 口径不变；⑥单测：ds_flatten_test 分表分流断言/全量补全维持/pg 不补全维持/use 随网/part_id 回归/merge 幂等/序列化往返 + pg 集三用例（分流汇总去重/O(1) 查询/序列化往返后 set 就绪）+ serialization_test 全族往返（含空容器）；⑦QA：test_emir_partition / test_emir_project_design NET_CONNECTIONS 读取改 NETS + DSNet 形态手算改写 + pg 集断言（空集形态 / debug db power={3}） | S9/S10 全链 | ✅（2026-09-13 完成） |

| D-SPLIT | **分区产物按 pg/信号拆分文件 + net id 0 专属 OBS + 对象名字符集修正（2026-09-14 用户裁定，见 design-db-plan.md S9 节「2026-09-14 分区产物拆分裁定」「net id 0 专属 OBS 裁定」与 §2.6 换算段补记）**：①产物四类 → 六类（`PART_{xp}_{yp}.{GEOMETRY,GEOMETRY_PG,NETS,NETS_PG,INSTANCES,INST_CONNECTIONS}`——GEOMETRY/NETS 各按 pg/信号拆两对象，信号大文件与 pg 小文件物理分离，④ 提取首期只加载 GEOMETRY_PG + NETS_PG；`DSPartitionNets` 单表化保留类壳两侧复用、`DSPartitionProduct` 拆 nets_/nets_pg_/geometry_/geometry_pg_ 四成员、merge_from 四表分别合并、crossing 跟随侧别、OBS 恒归信号侧、pg 判定源 = S5b is_pg_net；红线：拆文件不改语义）；②net 区间长度含 local 0 空洞位（count = 真网数 + 1、global = start + local 删 −1——S6/`DSHierTree::global_net_id`/S9 换算/S7/name mapper 全线同步；键 0 = OBS 专属位，obs 位判别降为防御校验，get_net(0) = None；net 总数口径 = id 跨度 − 空洞数〔正常 = 树节点数〕，S10 net 域 expected = Σ (net_count − 1)，DSGN::0022 文案同步）；③对象名字符集（DEVELOPMENT_GUIDELINES §2.9）：`partition_obj_name`/`id_map_index_obj_name`/`id_map_segment_obj_name` 三构造器 `/` → `.`；④连带：S9 merge 任务写六对象 + freeze keys 经 PARTITION_KINDS 自动跟随、S10 `ds_verify_partition` 六对象签名（两侧计数/素材合计口径不变）、NET 片段提取 = 两几何键集并集、debug get_net is_pg 路由单表查、`load_partition` 四元组 → 六元组、EXDSPartitionNets 键集视图 `ids`（signal_ids/pg_ids 删除）、EXDSPartitionProduct 访问器 `nets_pg`/`geometry_pg`（geometry_net_count 删除）；⑤单测：ds_flatten_test 拆分分侧断言 + 键 0 专属改写 + 全部 net id 手算 +1、ds_verify/ds_hier/ds_name_mapper/ds_union 期望值同步、ds_id_map_test 双键集断言；⑥QA：test_emir_partition（GEOMETRY_PG/NETS_PG 空对象形态 + n1 global 1 + OBS 专属桶断言）/ test_emir_project_design（VDD0 global 4 隔离于 GEOMETRY_PG + NETS_PG 六对象手算、debug db NET 段 0 空 + 1-4 全条目、get_net(0)/(2) 口径）断言强度不降 | S9/S10/D-DBG/D-NETRE 全链 | ✅（2026-09-14 完成） |
每批 TDD + ./fly.sh 验证 + 不 commit（审查后统一处理）；R 批次完成后全量回归（单测 + QA emir + 真实数据烟测复跑）。

> **历史快照声明**：本表 S7/S9/S10 行及各批次描述为当时计划口径；2026-09-13 连接 id 化（S5b 名字形态 → id + flags 六位）、D18 翻转（DSPowerPin 删除）、D29 数据结构清理落地后，**以 [design-db-plan.md](design-db-plan.md) 对应节的实施备注与裁定行为准**。
