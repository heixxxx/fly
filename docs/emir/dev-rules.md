...
- **入口参数校验范式（2026-09-17 裁定，翻转 2026-09-13 裁定 5 的对外语义）**——header 直接校验、错误直接 raise 终止、白名单严格模式：
  ...
  - **文件参数显式可读校验**：入口 Step 1 对每个路径调 `ensure_readable_file(path, api_name, param_name)`（不存在 → `FileNotFoundError`；不可读 → `PermissionError`；统一文案含 api 名/参数名/路径）。格式嗅探（`sniff_*_header`）不在入口执行——执行点在文件首个解析任务（见上「校验前置」，2026-09-17 三段式裁定修订）。
