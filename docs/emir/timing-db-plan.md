...
切块清单不设 `TMChunkPlan` temp 对象（随实修订）：`tm_plan_file_chunks`
为文件入口任务体内返回值（切完即用——同任务体内提交逐块解析），块数
以清单对象 `{chunk_count: N}` 落 timing db（合并链编排任务依赖全部清
单对象——齐备即全部文件入口完成；汇总任务读清单推导逐文件块数表，
两者消费后随 §19 清理责任单点删除）。
