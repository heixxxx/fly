...
- 起始编号沿层级树自根向下按深度优先序分配（每个 block instance 一段连续区间，区间长度 = 该 block 定义的 instance/net/via instance 计数；**net 区间长度含 local 0 空洞位——2026-09-14 裁定**：net 区间长度 = 真网数 + 1、换算直接相加无 −1，root 块空洞位 = global 0 = **OBS 专属位**（真网 global id 自 1 起，不再与分区几何 OBS 桶键混叠；每块一个空洞位为设计内合法空洞——**net 总数统计口径 = id 空间跨度 − 空洞数量**，空洞数量正常与树节点数相等，不得直接用 Σ(net_count) 虚增）；via 区间维持原语义不变）。
...
> **编排三段式（2026-09-17 用户裁定，dev-rules §3「建库 API 流程标准」）**
> ：`build_design_db` 函数体恒为三段——①轻量参数预处理（schema + 文件
> 存在性 + 建库 + alpha 写入；不读文件内容——lef/def 嗅探下放文件首个
> 解析任务）②提交唯一根任务 `_design_flow_task`（全部 S1–S8 阶段任务族
> 在其体内按流程顺序提交，目录见 ds_flow.py 模块 docstring）③顶层提交
> freeze `_freeze_design_task`（依赖固定标记 verify_report——全局校验
> 任务 inputs 含各分区校验结果 + pg_nets + id 映射段表 ⟹ 蕴含全链正式
> 对象）。S9/S10 分区链与校验链由 `_plan_partition_chain_task` 体内动态
> 提交（分区数运行时才知——「形状运行时才知」场景）。master 提交任务数
> O(1)，与输入规模无关；运行时规模临时对象（展开分片/校验结果/net_union
> slice/pg 片段/id 片段）由各自消费任务自清理——清理责任单点。
...
- **实施备注（2026-09-13 落地）**：
  ...
  - **local 0 对接形态（2026-09-13 连接 id 化裁定后更新）**：S5b 连接表为 id 形态——port 引用条目 = local 0 占位 + port 位（⑧）；对接键 = **同一 port 的全局 pin id 相等**（父侧 (子实例 local id, port pin 全局 id) × 子侧 (local 0, 同一 port pin id)——port pin 组合键 `"block_cell_name/port_name"` 注册于容器 pin hasher，全局唯一），S7 内部链路**零字符串匹配**；子实例 local id → 树 child 节点经 `self_global_id_`（= 父块 instance_start + local id）反查。
  - **2026-09-16 修订：对接键升维为 (子 block cell id, port pin id)**——裁定 3 后同名 port 跨 block 共享 pin id，单 pin id 键会把不同子定义的同名 port 网误并；对接键取定义级 port 身份（子块 cell id + pin id 两层索引），零字符串匹配不变。
...
- ~~叶实例的电源引脚坐标在展开时预计算☆（裁定 D18，⑫ 注入点直接可用）~~ **2026-09-13 裁定翻转（D18 翻转，DSPowerPin 删除）**：电源引脚坐标的消费者是 ④ 提取——按复现原则自 instance + transform + cell pin 几何**自取**（④ 输入口径备忘：提取从完整图形重建拓扑，无需原始点列——工业口径）；⑫ analysis 不读 design db，其注入经 ④ 的引脚↔图节点绑定链路随 ⑥ 矩阵传递——预存服务对象错位，撤销预展开。
...
- **2026-09-13 partition 网数据结构重组裁定（用户终态，补记②更新——②中 `/NET_CONNECTIONS` 部分被本条取代）**：①第四类分区对象更名 `/NET_CONNECTIONS` → **`/NETS`**，形态 = `DSPartitionNets{part_id, nets_, pg_nets_}`——**信号网 / pg 网分表**（`CMUnorderedMap<uint64_t, DSNet>` × 2）：单网聚合 `DSNet{net_id, use_, connections_}`（use 自 S5b `net_uses_` 随网写入、缺省 SIGNAL；连接条目 `DSNetConnEntry{inst_id, pin_id, flags 六位}`——网 id 由 DSNet 键承载，条目不再冗余）；信号网（use 非 POWER/GROUND）= 全量补全连接副本（本分区自足），pg 网 = 不补全（仅本分区 instance 副本相关条目——补记②的补全口径不变，仅换表组织）；`net_of` 两表查（未命中 nullptr）。原 `DSPartNetConnections`（连接列表 + `uses_` 旁挂 map）删除。②**INST_CONNECTIONS 保留**（两种不同遍历方式的双索引：instance 维度 / net 维度各一）——条目 `DSPartConnection{inst_id, net_global_id, pin_id, flags 六位}` 与 DSNetConnEntry 同款对齐（+端点网 id，instance 维度必需）；分区产物维持四类，不删除任何连接对象；S10 校验维持 iconn/nconn 双口径。③**全局 pg 网 id 集**（正式对象 `"pg_nets"`）：`DSPgNetSet{power_, ground_}` 两个 `CMUnorderedSet<uint64_t>`（用户定稿：O(1) 判定、power/ground 分开）——产出 = flatten 后汇总任务读各分区合并任务写出的 pg 片段（`ds_collect_pg_net_slice`：本区 pg_nets_ 表键按 use 分流）→ `finalize_from_flatten` 去重合并（同 pg 网跨分区副本去重）；消费 = debug API get_net 的 is_pg 快速判定 + Python `load_design_pg_nets` 的 is_power/is_ground/is_pg 查询口。**双口径注意**：全局集口径 = use ∈ {POWER, GROUND}；S5b `is_pg_net` 口径 = special ∨ use ∈ {POWER, GROUND}——special net 无 USE 语句时进分区 pg_nets_ 表（连接不补全）但**不入全局集**（review 2026-09-13 标注，两处 is_pg 语义差需消费方知悉）。几何不进 DSNet（仍在分区 GEOMETRY 对象按 net 组织）；S5b per-DEF 侧不动。（**2026-09-14 拆分裁定后续更新**：`DSPartitionNets` 两表单对象又被拆为 `NETS` / `NETS_PG` 两个独立正式对象、`net_of` 改单表查——见下条。）
- **2026-09-14 分区产物拆分裁定（用户终态，补记②再度更新——②中产物四类改为六类）**：①**GEOMETRY 与 NETS 各按 pg/信号网拆两个对象保存**——分区产物四类 → **六类**：`PART_{xp}_{yp}.GEOMETRY`（信号网几何）、`PART_{xp}_{yp}.GEOMETRY_PG`（pg 网几何）、`PART_{xp}_{yp}.NETS`（信号网连接，全量补全）、`PART_{xp}_{yp}.NETS_PG`（pg 网连接，不补全）、`PART_{xp}_{yp}.INSTANCES`、`PART_{xp}_{yp}.INST_CONNECTIONS`（后两类不变）。目的 = **信号网大文件与 pg 小文件物理分离**——④ 提取首期专注电源网络时只加载 GEOMETRY_PG + NETS_PG 两个小对象（即 ④ 首期输入全集）。②**结构形态**：`DSPartitionGeometry` / `DSPartitionNets` 类定义各一份、两侧复用同构实例（**保留类壳**：DSPartitionNets 单表化删 pg_nets_ 成员、各持 part_id_ + 本侧表；part_id 保留在 NETS 两侧，GEOMETRY 两侧不新增——保持现结构最小变化）；`DSPartitionProduct` 分片容器拆四成员（nets_ / nets_pg_ / geometry_ / geometry_pg_），merge_from 四表分别合并；net_of 两表查改单对象单表查 + 加载侧路由。③**crossing_nets_ 拆分跟随**：各几何对象自带自己网的 crossing 集（pg 网的 crossing 在 GEOMETRY_PG、信号网在 GEOMETRY）——S10 跨分区统计 = 两侧合计，口径不变。④**OBS 桶归信号侧 GEOMETRY**（OBS 是设计级阻挡非网数据；pg 侧保持纯净只含 pg 网条目）。⑤**pg 判定源 = S5b is_pg_net**（special ∨ use ∈ {POWER, GROUND}，与原 DSPartitionNets 分表口径一致）。⑥**连带面**：S9 每分区合并任务写六类对象（freeze final_keys 的 partition_keys 推导经 PARTITION_KINDS 自动跟随）；S10 ds_verify_partition 签名随六对象（两侧计数与素材合计，口径不变）；NET 片段提取 = GEOMETRY + GEOMETRY_PG 两键集并集；debug API get_net 路由 = 全局 pg 集 is_pg 判定 → 加载对应侧 NETS 或 NETS_PG 单表查；`load_partition` 返回四元组 → 六元组（与 PARTITION_KINDS 序对齐）；对象名分隔符 `PART_{xp}_{yp}/{kind}` → `PART_{xp}_{yp}.{kind}`（同日对象名字符集裁定，见 DEVELOPMENT_GUIDELINES §2.9）。**红线：拆文件不改语义**——pg 不补全/信号全量补全/OBS/键 0 语义/flags 六位/数位统计全部口径不变，仅物理文件拆分。
- **2026-09-14 net id 0 专属 OBS 裁定（用户终态，消除键 0 混叠复杂度）**：net 区间改为**长度含 local 0 空洞位**形态（count = 真网数 + 1、start 紧凑累计——与 instance 的 local 0 占位形态完全同构），换算 `global = net_start + local` **删除 −1**（S6 分配、DSHierTree::global_net_id、S9 展开换算、S7 归并、name mapper net 维度全线同步——见 §2.6 换算段补记）；root 块 start 0 的空洞位 = **global 0 = OBS 专属位**，全部真网 id 不再与分区几何 OBS 桶键混叠：obs 位判别保留为防御校验（`net_entries(0)` 恒空、`obs_entries()` = 键 0 全桶），get_net docstring 的键 0 混叠例外条款删除（root 首网现 global 1，无几何时映射查不到 → 常规 None 路径；get_net(0) = None——id 0 非真网）。**net 计数口径（同日裁定 3）**：net 总数 = id 空间跨度 − 空洞数量（空洞数正常 = 树节点数，每块一个 local 0 空洞位含 root）——S10 net 域 expected = Σ (net_count − 1)（空洞位不占 expected），DSGN::0022 消息中 net 域合法空洞说明同步（每块空洞位为设计内合法、不计入 expected/holes）。via 区间不动（无混叠问题）。
- **实施备注（2026-09-13 落地）**：
  - **结构与算法（2026-09-14 拆分裁定终态同步）**：`src/emir/design/cpp/ds_flatten.h/.cpp`（`DSNetConnEntry`/`DSPartConnection`/`DSGeomEntry`/`DSPartitionGeometry`/`DSPartInstances`/`DSPartInstConnections`/`DSNet`/`DSPartitionNets`（单表化，NETS/NETS_PG 两侧复用同构实例）/`DSPgNetSlice`/`DSPgNetSet` + 六类正式对象 + `DSPartitionProduct` 分片聚合容器（nets_/nets_pg_/geometry_/geometry_pg_ 四成员；merge_from 追加合并，四表分别合并、use 同键覆盖）+ `ds_flatten_block` 每定义一调用（pg/信号分侧分流写入、OBS 恒归信号侧）+ `ds_collect_pg_net_slice`（提取源 = NETS_PG 侧对象））+ 导出面（EXDS* + ds_flatten_block + ds_build_pg_net_set）。复合放置变换存入树节点（`DSHierNode.composite_transform_`，S6 DFS 递推回填）——展开任务只读单一 def 产物即可拿到任意出现位置的复合变换，保证「每份 DEF 数据只读一次」。归属判定 `assign_point`：core 半开区间包含 → primary（恰一）；无 core 命中（实例越出 DIEAREA 的输入形态）防御回退距最近 core 分区（tie 取小 id），恰一不变式恒成立；副本集 = extend 包含放置点的全部分区。UNPLACED 实例无物理放置、不入分区产物（D14 同族口径）。
  - **连接副本形态（2026-09-13 重组终态同步）**：NET 维度条目 `DSNetConnEntry{inst_id, pin_id, flags 六位}` 挂所在 `DSNet`（网 id 由键承载）；INST 维度条目 `DSPartConnection{inst_id, net_global_id, pin_id, flags 六位}` 跟随实例副本（instance 维度组织必需端点网 id）；port 引用 → 端点实例 = 所属块实例自身 global id（⑧ local 0 映射）+ port 位，pin 全局平铺 id 直存（S9 连接处理零字符串匹配，flags 位自 S5b 条目整体直拷）。信号网连接跟随全部 net 副本（每几何命中分区一份全量列表）；pg 网判定 = special net 或 USE POWER/GROUND（S5b 连接解析节点记录进 `DSNetBuildData.pg_nets_`，2026-09-13 修正：map 充当 set 妥协回归 `CMUnorderedSet` 直存——序列化宏已接 set 全族），pg 网 NETS_PG 侧对象仅收本分区 instance 副本相关条目（2026-09-14 拆分裁定：pg_nets_ 表随单表化删除，落点为独立对象）。
  - **net id 0 语义（2026-09-14 空洞位裁定取代旧口径）**：net 区间长度 = 真网数 + 1（local 0 空洞占号）、start 紧凑累计、global = net_start + local（无 −1）；root 块空洞位 = global 0 = **OBS 专属**（不再与真网同键共存），真网 global id 从 1 起、每块一个设计内空洞（与 instance 的 local 0 占位同构）。obs 位保留为防御校验。
  - **per-net use（2026-09-13 USE 全量补收裁定 + 同日重组终态同步）**：use 随 `DSNet.use_` 写入（flatten 时自 `DSNetBuildData.net_uses_` 换 global id、缺省 SIGNAL——S5b 只记非 SIGNAL 条目，读取口径一致）、跟随 net 副本（仅几何命中分区；无几何网无副本），merge_from 同键覆盖幂等。get_net 类 debug 消费直接读分区产物、不查 S5b per-DEF 产物。
  - **id → partition 反向映射（2026-09-13 debug 定位裁定，结构 `ds_id_map.h/.cpp`）**：按 id 直接索引的 `CMVector<uint32_t>`（值 = partition id，复用 `DSSubPartition.partition_id_`），按 id 区间**分段落盘按需加载**——段粒度 2^20 id（段对象 = 定长 pids 数组 ≈ 4 MiB，空洞条目 = kIdMapNoPartition 哨兵；空洞段省略存储、段表不登记）。inst/net 各一张全空间映射，三层结构：`DSIdPartitionSlice`（S9 每分区合并任务写本区片段的临时对象——INST = primary 副本 id 集〔依赖恰一 primary 不变式，不重复全空间校验〕/ NET = GEOMETRY + GEOMETRY_PG 两对象键集并集〔2026-09-14 拆分裁定，net 副本口径不变〕+ 本区 pid）→ `DSIdPartitionSegment` 段正式对象（`id_partition_map.{INST,NET}.S{段号}`）→ `DSIdPartitionIndex` 段表轻对象（`id_partition_map.{INST,NET}`，非空段起始 id 升序；对象名分隔符 `.`——2026-09-14 对象名字符集裁定）。flow：分区合并任务追加写片段 → `_id_map_merge_task`（INST/NET 各一，S9 plan 动态提交）读全部片段 → `ds_merge_id_partition_slices` 排序线性分段 → 段对象逐段写定 + 段表最后写定（freeze 依赖段表即依赖全部段对象）→ 片段清理。消费 = debug 读库 API（§6）按 id 定位分区后整分区对象按需加载（进程内 LRU）。
  ...
  - **两级任务编排（ds_flow.py）**：plan 任务（依赖 S8 分区表 + S6 树 + block 名清单 + alpha；分组信息依赖树运行时数据，故在 worker 上**动态提交**下游任务——同 solver kickoff 先例）：预估 = DEF 文件大小 × 树上实例化次数，≥ alpha `def_aggregate_threshold` 独占任务、低于阈值按 def_paths 序贪心聚合（D26）→ per-组展开任务并行（每组只读本组 def 产物 + 每任务对全部分区各写一份分片临时对象，未触达分区写空产物保证合并依赖恒可解）→ 每分区一合并任务（merge 本分区全部分片 → 六类正式对象唯一写定）→ freeze（final_keys = 静态正式对象 + 运行时确定的全部分区对象名，由 plan 动态提交）。
...
- **实施备注（2026-09-13 落地）**：
  - **结构与算法**：`src/emir/design/cpp/ds_verify.h/.cpp`（`DSIdDomain` id 连续性观测域 + `DSPartitionCheckResult` 分区级校验结果——计数 + 全局校验素材 id 集 + `DSDesignCheckReport` 全局报告——损坏类三字段 union_inconsistency_/coverage_gap_/namemap_inconsistency_ 非空即 fatal（`ds_verify_report_or_fatal` 逐项处置，DSGN::0019/0020/0021 码 80）+ 观测类三域 + density_variance_（DSGN::0022/0023）+ 全局统计（DSGN::0024 INFO）+ `ds_verify_partition`（每分区一调用，只读本分区六类正式产物——2026-09-14 拆分裁定签名随六对象，两侧计数/素材合计口径不变）/ `ds_verify_design`（纯函数不直接 fatal——损坏类写报告字段，处置权在调用方））+ 导出面（EXDSIdDomain/EXDSPartitionCheckResult/EXDSDesignCheckReport + 三函数）+ `load_design_verify_report`（R9 wait_obj 形态）+ `VERIFY_REPORT_OBJ = "verify_report"`。
  - **校验口径**（写清精确对齐）：①instance 域 expected = Σ 树节点 instance_count − (非 root 节点数) − 1（扣除各非 root 定义的 local 0 保留槽与 root 自身 global 0——root 无实体副本不入分区）；actual = 全部分区 INSTANCES distinct id 并集；duplicates = 多 primary 直方图超量（UNPLACED 实例是合法空洞）。②net 域 expected = 全域上界 − 空洞数 = Σ (net_count − 1)（2026-09-14 裁定：net 区间长度含每块一个 local 0 空洞位——设计内合法空洞不占 expected；**net 总数统计口径 = id 空间跨度 − 空洞数量**，空洞数量正常 = 树节点数，不得直接用 Σ(net_count) 虚增）；actual = 分区产物覆盖的 distinct 真网 id（两几何对象含非 OBS 条目的键——键 0 OBS 专属位不计 + 两 crossing + 两侧连接表键与端点引用）；空网（无几何无连接）是合法空洞。③via 域 actual = per-DEF 网产物 via_instances_ 键经 (节点 via_start + local − 1) 换算的并集（via instance 不入分区产物，权威存储只在 S5b 产物）。④密度守恒 primary 口径 = Σ 各分区 primary 计数 vs Σ 首份定义 (stats.instance_count − stats.unplaced_count)（树展开可放置实例总数——leaf/各级 block 实例/fake cell 实例恰一 global id 恰一 primary；UNPLACED 扣除；重名定义保留首份与树构建同语义）。⑤覆盖校验 = 分区 core 行列网格性质（切线集一致 + (xp,yp) 完整恰一次 + core 边界恰为切线段）+ 外沿与切线对齐 global_density 格网覆盖域（int64 域运算防 cols×bin 溢出；空表 + 未配置格网放行）。⑥namemap = cell/pin/via cell/layer 四全局 hasher + 每伴生 instance/net hasher 全查双向闭环（for_each (name,id) → get_name(id) == name）；树节点引用的 block 定义不在名字表并入 namemap 损坏描述。
  - **flow 挂接形态**：校验链由 S9 plan 任务动态提交（merge 之后、freeze 之前——与 freeze 同一动态链）：每分区一校验任务并行（inputs 只声明本分区四类对象——不跨区读红线）→ 全局汇总校验任务（读全部校验结果临时对象 + 树 + DSDesign + stack + global_density + net_union + per-DEF blocks/nets/names）→ freeze（final_keys 增 verify_report；清理清单 = 静态临时键集 + 展开分片 + 校验结果临时对象〔2026-09-17 §19 批次：键固定名 __dsn__{name}、清单任务内静态构造、严格 remove；pg/id/net_union 片段由各自汇总任务自清理，不进 freeze 清单〕）。损坏类 fatal = 进程码 80 退出 + master 联动，report 不落盘、freeze 依赖缺失——损坏库不冻结（fatal 语义 + 依赖挂接双保险）；观测类 warn 不阻断。
...
| `load_design_pg_nets(db)` | 全局 pg 网 id 集（is_power/is_ground/is_pg O(1) 查询 + power_count/ground_count——2026-09-13 重组裁定，正式对象 `"pg_nets"`） |
...
| `get_net(net)` | 入参 global id 或层级路径名：net id/层级名/use 属性（八值字符串，缺省 SIGNAL——2026-09-13 重组裁定自分区 NETS 的 `DSNet.use_` 直取）/connections 数量/driver/receiver/hybrid 数（三分类互斥单列）与 power/ground/clock/port 端点计数（数连接 flags 位，零推导）；非 pg 网附 connections 明细（端点实例 id+层级名+pin id+pin 名+flags 概要——port 条目如实呈现为块实例名+port 名）；**pg 网（全局 pg 网 id 集 `"pg_nets"` 命中，O(1) 判定）不返回明细**，且 **is_pg 判定即路由键**——pg 网读 `PART_{xp}_{yp}.NETS_PG`、信号网读 `PART_{xp}_{yp}.NETS` 单表查（2026-09-14 拆分裁定）。无几何副本的网、**有几何但无任何连接的悬浮网**（DSNet 不落表）、id 0（OBS 专属位、非真网 id）、或 special 无 USE 网（入 NETS_PG 但全局 pg 集不含——is_pg 路由至信号侧不命中）返回 None（review 2026-09-13 显式口径——旧兜底曾静默降级为 SIGNAL 空概要；键 0 混叠例外条款随 2026-09-14 OBS 专属裁定删除） |
| `get_layer(layer)` | id/name/type/direction/width/pitch/spacing/min_area |
| `convert_to_id(kind=…, name=…)` | name → id；kind ∈ {cell, pin, layer, via_cell, inst, net}——inst/net 走层级路径 name mapper、pin 走组合键 `"cell_name/pin_name"`（D1）、其余走 hasher；支持 kwargs 直写形态 `convert_to_id(cell="INV_X1")`；未命中 None。**2026-09-16 裁定 3 注记**：pin 改裸 pin 名直查（同名 pin 跨 cell 共享 id），旧 `"cell_name/pin_name"` 组合形态兼容（取名段） |
| `convert_to_name(kind=…, id=…)` | id → name（与上对称；用户示例 kwargs 形态 `convert_to_name(inst=10)`）；inst/net 返回层级全路径名、pin 返回组合键全名；未命中 None。**2026-09-16 裁定 3 注记**：pin 返回裸 pin 名 |

（映射对象名 API 与段常量在 `DesignDb`：`id_map_index_obj_name(kind)` = `id_partition_map.{kind}`、`id_map_segment_obj_name(kind, k)` = `id_partition_map.{kind}.S{k}`、`ID_MAP_SEGMENT_BITS = 20`——对象名分隔符 `.` 系 2026-09-14 对象名字符集裁定（仅字母/数字/`.`/`_`，DEVELOPMENT_GUIDELINES §2.9），分区对象名同步为 `PART_{xp}_{yp}.{kind}`。）
...
| D18 | ~~实例电源引脚坐标展开时机~~ **2026-09-13 裁定翻转：A/B 均撤销，DSPowerPin 结构删除** | A. S9 切分时预展开进分区（⑫ 直接消费，曾裁定 A 并实施）；B. ⑫ 消费时现算。**翻转依据**：电源引脚坐标的消费者是 ④ 提取（按复现原则自 instance + transform + cell pin 几何自取——④ 输入口径备忘：提取从完整图形重建拓扑，无需原始点列，工业口径）；⑫ analysis 不读 design db，注入经 ④ 的引脚↔图节点绑定链路随 ⑥ 矩阵传递——预展开产物（`DSInstance.power_pins_`）全流程无消费者，删除 |
...
| D29 | **数据结构清理批次（2026-09-13 裁定，已实施）**：删除无消费者/链路错位字段 + S5b 连接表彻底 id 化 | **删除项（四字段一结构）**：①DSPowerPin 结构（D18 翻转，见上）；②`DSInstance.weight_`——DEF WEIGHT 全流程无消费者（⑩ 功耗走 lib 表×翻转率、⑫ 走引脚电流），10⁹ 实例 × 8B ≈ 8GB 冗余；③`DSCell.def_units_per_micron_`/`class_`/`site_`——def_units 换算解析期完成无下游消费，class/site 无 EMIR 流程消费（种类判定用 flags 位）；④`DSBlockBuildData.fake_cells_` 副本——fake cell 数据经独立临时对象传 S5a 汇总并入全局表（产物本体不再冗余存一份；`fake_name_to_id_` 登记表保留）。**改造项**：S5b 连接表 id 化 + flags 六位（见 S5b/S7/S9 节 2026-09-13 条目）——id 换算在解析边界一次完成，S7/S9 内部链路零字符串匹配；序列化变更直接改（早期无兼容负担） |
