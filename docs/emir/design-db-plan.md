...
| `get_net(net)` | 入参 global id 或层级路径名：net id/层级名/use 属性（八值字符串，缺省 SIGNAL——2026-09-13 重组裁定自分区 NETS 的 `DSNet.use_` 直取）/connections 数量/driver/receiver/hybrid 数（三分类互斥单列）与 power/ground/clock/port 端点计数（数连接 flags 位，零推导）；非 pg 网附 connections 明细（端点实例 id+层级名+pin id+pin 名+flags 概要——port 条目如实呈现为块实例名+port 名）；**pg 网（全局 pg 网 id 集 `"pg_nets"` 命中，O(1) 判定——2026-09-13 重组裁定）不返回明细**。无几何副本的网，或**有几何但无任何连接的悬浮网**（DSNet 不落表）返回 None（review 2026-09-13 显式口径——旧兜底曾静默降级为 SIGNAL 空概要） |
