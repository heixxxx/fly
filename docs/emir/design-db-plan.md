...
**S1 tech lef 解析 → 层表（Stack 独立类型）+ tech 级通孔定义**
...
**S2 cell lef 解析 → 单元结构（每文件一并行任务 + 全局汇总，无业务合并）**
...
**S3 lib cell 与 lef cell 结构 merge（S5a 前置；2026-09-09 裁定 ⑯）**
...
**S4 DEF 头部轻量扫描 → block cell / port（每文件一并行任务 + 全局汇总，无业务合并）**
...
**S4b DEF via 定义解析（每 DEF 一任务；2026-09-09 裁定 ⑪/⑫）**
...
**S5a DEF 第一段：实例全量解析 ∥ 网名扫描（每 DEF 两任务并行，无业务合并；2026-09-09 裁定）**
...
**S5b DEF 第二段：网内容全量分批解析（每 DEF 一任务，内部分批多阶段；依赖 S5a + S4b；2026-09-09 裁定 ③/⑨/⑪）**
...
**S9 flatten 展平 + 分区保存（展开任务按 block 定义切分 + 每分区合并任务；2026-09-09 裁定）**
